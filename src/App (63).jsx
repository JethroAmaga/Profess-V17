...
export default function Profess() {
    ...
    if (stage === "intro") return (
      <Overlay footer={
        ...
        <motion.h1
          ...
          style={{ fontFamily: "'Playfair Display',serif", fontStyle: "italic", fontWeight: 400, fontSize: "clamp(44px,9vw,84px)", color: "#F0EDE6", marginBottom: "18px", letterSpacing: "-0.025em", lineHeight: 1.05 }}>
  ...
  return (
    <div onPointerMove={handlePointerMoveParallax}
      style={{ minHeight: "100vh", height: "100vh", overflow: "hidden", background: BG, color: "#F0EDE6", fontFamily: "'Inter',sans-serif", fontWeight: 300, position: "relative" }}>
      <style>{css}</style>
      ...
      {/* Cursor spotlight — a soft pool of light over the photograph that
          follows the pointer, present from the very first screen (intro,
          before "Begin") so the backdrop feels touchable immediately. */}
      {!isMobile && (
        <motion.div style={{ position: "absolute", inset: 0, zIndex: 1, pointerEvents: "none",
          mixBlendMode: "soft-light", background: spotlightBg }} />
      )}

      {/* Ambient sound toggle — visible from intro, before Begin */}
      <motion.button onClick={() => setAmbientOn((v) => !v)}
        initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.3 }}
        whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.94 }}
        aria-label={ambientOn ? "Mute ambient sound" : "Enable ambient sound"}
        style={{ position: "absolute", top: "20px", right: "20px", zIndex: 40, width: "32px", height: "32px",

...
export default function Profess() {
  ...
  function TiltBox({ children, style }) {
    ...
    if (isMobile) return <div style={style}>{children}</div>;
    const handleMove = (e) => {
          ...
          <AnimatePresence>
            {transcriptOpen && (
              <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }}
                transition={{ duration: 0.3, ease: [0.22,1,0.36,1] }}
                style={{ position: "absolute", left: isMobile ? "20px" : "56px", top: isMobile ? "calc(4% + 28dvh + 80px)" : "72px", bottom: isMobile ? "150px" : "150px",

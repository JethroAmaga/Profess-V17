...
export default function Profess() {
  ...
  const css = `
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    /* Blinking — eyes close briefly every ~4s */
    @keyframes blink{0%,94%,100%{transform:scaleY(1)}96%,98%{transform:scaleY(0.08)}}
    @keyframes blinkDelay{0%,90%,100%{transform:scaleY(1)}92%,94%{transform:scaleY(0.08)}}

    /* Eye movement — subtle horizontal drift */
    ...
    @keyframes eyeDriftSlow{0%,100%{transform:translateX(0)}25%{transform:translateX(-1.5px)}55%{transform:translateX(1px)}85%{transform:translateX(-0.5px)}}

    /* Gesture — hand raise for emphasis */
    ...
    /* Micro-expression — quick brow flash */
    ...
    .screen-enter{animation:screenFade .3s ease forwards}
  `;

  // ── LANGUAGE SELECTION SCREEN ──────────────────────────────────────────
  if (screen === "lang") return (
    <div className="screen-enter" style={{ minHeight:"100vh", background:"#0C0C0C", color:"#F0EDE6", fontFamily:"'Inter',sans-serif", fontWeight:300, display:"flex", flexDirection:"column" }}>
      <style>{css}</style>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"48px", padding:"40px" }}>
        ...
        <div style={{ display:"flex", gap:"24px", flexWrap:"wrap", justifyContent:"center" }}>
          <button onClick={() => { setLang("en"); setScreen("mode"); }} style={{ background:"none", border:"1px solid #2A2520", color:"#F0EDE6", fontFamily:"inherit", padding:"32px 36px", cursor:"pointer", width:"200px", textAlign:"left", transition:"all .25s", borderRadius:"2px" }}
  ...
  if (screen === "intensity") {
    ...
    const accentColor = isFormal ? "#C8B89A" : "#C890B0";
    const levels = isID ? [
      { key:"comfortable", icon:"🌱", label:"Nyaman", desc:"Supportif dan membangun — cocok untuk pemula atau mencoba skenario baru" },
      { key:"challenging", icon:"⚡", label:"Menantang", desc:"Ketat dan jujur — standar tinggi, feedback langsung tanpa basa-basi" },
      { key:"no_mercy",   icon:"🔥", label:"Tanpa Ampun", desc:"Tekanan maksimal — untuk yang ingin diuji habis-habisan" },
    ] : [
      { key:"comfortable", icon:"🌱", label:"Comfortable", desc:"Supportive and constructive — good for trying new scenarios or warming up" },
      { key:"challenging", icon:"⚡", label:"Challenging",  desc:"Rigorous and honest — high standards, direct feedback, no softening" },
      { key:"no_mercy",   icon:"🔥", label:"No Mercy",     desc:"Maximum pressure — for when you want to be pushed to your absolute limit" },
    ];
    return (
      <div style={{ minHeight:"100vh", background:"#0C0C0C", color:"#F0EDE6", fontFamily:"'Inter',sans-serif", fontWeight:300, display:"flex", flexDirection:"column" }}>
        <style>{css}</style>
        <div style={{ padding:"28px 32px 20px", borderBottom:"1px solid #1A1A1A", display:"flex", alignItems:"center", gap:"14px" }}>
          <span style={{ fontFamily:"'Playfair Display',serif", fontSize:"20px", fontWeight:500, letterSpacing:".04em" }}>Profess</span>
          <button onClick={() => setScreen("disclaimer")} style={{ marginLeft:"auto", background:"none", border:"none", color:"#3A3530", fontSize:"11px", cursor:"pointer", letterSpacing:".05em", textTransform:"uppercase" }}>← {isID?"Kembali":"Back"}</button>
        </div>
        <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"36px", padding:"40px 24px" }}>
          <div style={{ textAlign:"center" }}>
            <p style={{ fontFamily:"'Playfair Display',serif", fontSize:"18px", color:"#5A5550", marginBottom:"6px" }}>{isID?"Pilih intensitas sesi":"Choose your intensity"}</p>
            <p style={{ fontSize:"12px", color:"#3A3530", letterSpacing:".06em" }}>{isID?"Ini menentukan seberapa keras Profess mendorong kamu":"This determines how hard Profess will push you"}</p>
          </div>
          <div style={{ display:"flex", flexDirection:"column", gap:"12px", width:"100%", maxWidth:"420px" }}>
            {levels.map(lvl => (
              <button key={lvl.key} onClick={() => { setIntensity(lvl.key); setScreen("scenario"); }}
                style={{ background:"none", border:"1px solid #2A2520", color:"#F0EDE6", fontFamily:"inherit", padding:"20px 24px", cursor:"pointer", textAlign:"left", transition:"all .2s", borderRadius:"2px", display:"flex", gap:"16px", alignItems:"flex-start" }}
                onMouseEnter={e=>{e.currentTarget.style.borderColor=accentColor;e.currentTarget.style.background="#141210";}}
                onMouseLeave={e=>{e.currentTarget.style.borderColor="#2A2520";e.currentTarget.style.background="none";}}>
                <span style={{ fontSize:"20px", flexShrink:0 }}>{lvl.icon}</span>
                <div>
                  <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"16px", color:accentColor, marginBottom:"4px" }}>{lvl.label}</div>
                  <div style={{ fontSize:"12px", color:"#5A5550", lineHeight:1.6 }}>{lvl.desc}</div>
                </div>
              </button>
            ))}
          </div>
  ...
  if (screen === "scenario") {
      ...
      formal: {
        id: [
          { group:"Akademik", items:[
            "Sidang Skripsi — Penguji yang Skeptis",
            "Presentasi Seminar — Dosen yang Tidak Yakin",
            "Debat Parlemen Asia — Mosi Kontroversial",
            "Ospek Organisasi — Senior yang Menguji Mental",
          ]},
          { group:"Karir", items:[
            "Interview Kerja Pertama — HRD yang Kritis",
            "Pitching Startup ke Investor — 5 Menit untuk Meyakinkan",
            "Negosiasi Gaji — Atasan yang Tidak Mudah",
            "Press Conference — Jurnalis yang Agresif",
            "Rapat dengan Klien — Keputusan di Tangan Mereka",
          ]},
          { group:"Hukum & Publik", items:[
            "Persidangan Mock Trial — Jaksa yang Tidak Memberi Celah",
            "Debat Publik — Lawan yang Lebih Berpengalaman",
            "Audiensi dengan Pejabat — Birokrasi yang Tidak Berpihak",
          ]},
        ],
        en: [
          { group:"Academic", items:[
            "Thesis Defense — The Skeptical Examiner",
            "Seminar Presentation — The Unconvinced Professor",
            "Asian Parliamentary Debate — A Controversial Motion",
            "Org Initiation — Senior Testing Your Limits",
          ]},
          { group:"Career", items:[
            "First Job Interview — The Critical HRD",
            "Startup Pitch — 5 Minutes to Convince an Investor",
            "Salary Negotiation — A Manager Who Won't Budge",
            "Press Conference — An Aggressive Journalist",
            "Client Meeting — The Decision Is Theirs",
          ]},
          { group:"Legal & Public", items:[
            "Mock Trial — A Prosecutor With No Mercy",
            "Public Debate — An Opponent More Experienced Than You",
            "Government Audience — Bureaucracy That Isn't On Your Side",
          ]},
        ],
      },
      social: {
        id: [
          { group:"Relasi", items:[
            "Reuni Teman Lama — Ada yang Belum Selesai",
            "Ketemu Crush — Kesempatan yang Tidak Bisa Diulang",
            "Kenalan Baru di Acara — Tidak Ada yang Dikenal",
            "Minta Maaf ke Teman — Sudah Terlalu Lama Didiamkan",
            "Konfrontasi Teman — Sebelum Hubungan Ini Berakhir",
          ]},
          { group:"Keluarga", items:[
            "Bicara Jujur ke Orang Tua — Tentang Masa Depan",
            "Ketemu Calon Mertua — Pertama Kali",
            "Adik yang Tidak Mau Diatur — Tapi Kamu Peduli",
          ]},
          { group:"Profesional Casual", items:[
            "Ngobrol dengan Senior di Kantor — Yang Kamu Kagumi",
            "Mengkritik Atasan dengan Sopan — Tanpa Kehilangan Posisi",
            "Networking di Acara — Mulai dari Nol",
          ]},
          { group:"Situasional", items:[
            "Kenalan di Bookstore — Buku yang Sama",
            "Perjalanan Panjang — Teman Duduk yang Menarik",
            "Golf dengan Pengusaha Senior — Empat Jam untuk Berkesan",
          ]},
        ],
        en: [
          { group:"Relationships", items:[
            "Reconnecting with an Old Friend — Something Was Left Unsaid",
            "Meeting Your Crush — A Chance You Can't Repeat",
            "New People at an Event — You Don't Know Anyone",
            "Apologizing to a Friend — You've Waited Too Long",
            "Confronting a Friend — Before This Ends",
          ]},
          { group:"Family", items:[
            "Honest Talk with Parents — About Your Future",
            "Meeting the Parents — First Time",
            "A Sibling Who Won't Listen — But You Care",
          ]},
          { group:"Professional Casual", items:[
            "Talking to a Senior You Admire — At the Office",
            "Giving Feedback to Your Boss — Without Losing Ground",
            "Networking at an Event — Starting From Zero",
          ]},
          { group:"Situational", items:[
            "Bookstore Encounter — The Same Book",
            "Long Journey — An Interesting Seatmate",
            "Golf with a Senior Executive — Four Hours to Make an Impression",
          ]},
        ],
      },
    };
    ...
    return (
      <div className="screen-enter" style={{ minHeight:"100vh", background:"#0C0C0C", color:"#F0EDE6", fontFamily:"'Inter',sans-serif", fontWeight:300, display:"flex", flexDirection:"column" }}>
        <style>{css}</style>
        <div style={{ padding:"20px 28px", borderBottom:"1px solid #1A1A1A", display:"flex", alignItems:"center", gap:"14px" }}>
          <span style={{ fontFamily:"'Playfair Display',serif", fontSize:"20px", fontWeight:500, letterSpacing:".04em" }}>Profess</span>
          <button onClick={() => setScreen("intensity")} style={{ marginLeft:"auto", background:"none", border:"none", color:"#3A3530", fontSize:"11px", cursor:"pointer", letterSpacing:".05em", textTransform:"uppercase" }}>
            ← {isID ? "Kembali" : "Back"}
          </button>
        </div>
        <div style={{ flex:1, overflowY:"auto", padding:"32px 28px" }}>
          <div style={{ maxWidth:"600px", margin:"0 auto", display:"flex", flexDirection:"column", gap:"32px" }}>
            <div>
              <p style={{ fontFamily:"'Playfair Display',serif", fontSize:"18px", color:"#5A5550", marginBottom:"6px" }}>
                {isID ? "Pilih skenario" : "Choose a scenario"}
              </p>
              <p style={{ fontSize:"12px", color:"#3A3530", letterSpacing:".05em" }}>
            ...
            <button
              ...
              style={{ background:"none", border:`1px solid ${accent}`, color:accent, fontFamily:"inherit", fontSize:"12px", padding:"14px 20px", cursor:"pointer", textAlign:"left", letterSpacing:".05em", textTransform:"uppercase", transition:"all .2s" }}
            ...
            {groups.map((group, gi) => (
              <div key={gi} style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
                <p style={{ fontSize:"9px", color:"#3A3530", letterSpacing:".12em", textTransform:"uppercase", marginBottom:"4px" }}>
                  {group.group}
                </p>
                {group.items.map((item, ii) => (
                  <button key={ii}
                    onClick={() => { setScenario(item); startSession(pendingMode, item); }}
                    style={{ background:"none", border:"1px solid #1E1E1E", color:"#C0BDB8", fontFamily:"inherit", fontSize:"13px", padding:"12px 16px", cursor:"pointer", textAlign:"left", transition:"all .2s", lineHeight:1.5 }}
                    onMouseEnter={e=>{ e.currentTarget.style.borderColor=accent; e.currentTarget.style.color="#F0EDE6"; e.currentTarget.style.background="#141210"; }}
                    onMouseLeave={e=>{ e.currentTarget.style.borderColor="#1E1E1E"; e.currentTarget.style.color="#C0BDB8"; e.currentTarget.style.background="none"; }}>
                    {item}
                  </button>
                ))}
              </div>
    ...
    return (
      <div style={{ minHeight:"100vh", background:"#0C0C0C", color:"#F0EDE6", fontFamily:"'Inter',sans-serif", fontWeight:300, display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"40px 24px", gap:"32px" }}>
          <div style={{ maxWidth:"520px", width:"100%", display:"flex", flexDirection:"column", gap:"28px" }}>
            <div>
              <p style={{ fontSize:"11px", color:"#4A4540", letterSpacing:".1em", textTransform:"uppercase", marginBottom:"8px" }}>{isID?"Ringkasan Sesi":"Session Complete"}</p>
              <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"22px", fontWeight:400, color:accentColor }}>
                {isID?"Kamu sudah selesai.":"You showed up."}</h2>
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:"14px" }}>
              {summaryLines.map((line, i) => {
                ...
                return (
                  <div key={i} style={{ display:"flex", gap:"12px", alignItems:"flex-start" }}>
                    <div style={{ width:"6px", height:"6px", borderRadius:"50%", background:accentColor, flexShrink:0, marginTop:"7px", opacity:.7 }}/>
                    <p style={{ fontSize:"14px", lineHeight:1.8, color:"#C0BDB8" }}>{clean}</p>
                  </div>
                );
              })}
            </div>
            <div style={{ borderTop:"1px solid #1E1E1E", paddingTop:"24px", display:"flex", gap:"12px", flexWrap:"wrap" }}>
              <button onClick={() => { setSummary(null); setScreen("session"); }}
                style={{ background:"none", border:`1px solid ${accentColor}`, color:accentColor, fontFamily:"inherit", fontSize:"11px", padding:"10px 22px", cursor:"pointer", letterSpacing:".08em", textTransform:"uppercase", transition:"all .2s" }}
                onMouseEnter={e=>{e.currentTarget.style.background=accentColor;e.currentTarget.style.color="#0C0C0C";}}
                onMouseLeave={e=>{e.currentTarget.style.background="none";e.currentTarget.style.color=accentColor;}}>
                {isID?"Lanjut sesi":"Continue session"}
              </button>
              <button onClick={resetSession}
                style={{ background:"none", border:"1px solid #2A2520", color:"#6A6560", fontFamily:"inherit", fontSize:"11px", padding:"10px 22px", cursor:"pointer", letterSpacing:".08em", textTransform:"uppercase", transition:"all .2s" }}
                onMouseEnter={e=>{e.currentTarget.style.borderColor="#4A4540";e.currentTarget.style.color="#8A8580";}}
                onMouseLeave={e=>{e.currentTarget.style.borderColor="#2A2520";e.currentTarget.style.color="#6A6560";}}>
  ...
  const charAnimBlock = (size) => {
    ...
    return <div key={displayRole+currentMood} className="char-enter" style={{ width:size, height:size==="170px"?"215px":"80px", opacity:isTransitioning?0:1, transition:"opacity .38s", animation:talkAnim||moodAnim||idleAnim, filter:isSpeaking?`drop-shadow(0 0 ${size==="170px"?"14px":"8px"} ${charMeta.accent}50)`:"none" }} dangerouslySetInnerHTML={{ __html:charSVG }}/>;
  };
  ...
  const msgList = (fontSize="14px") => messages.map((msg,i) => {
    ...
    const mc = isA
      ? (mInRole
          ? (charCache[currentRole]||CHARS.default)
          : (currentRole !== "default" ? (charCache[currentRole]||CHARS.default) : CHARS.default))
      : CHARS.default;
      ...
      <div key={i} className="msg-enter" style={{ display:"flex", flexDirection:"column", gap:"5px" }}>
        ...
        {isA && segments ? (
          <div style={{ display:"flex", flexDirection:"column", gap:"4px" }}>
            {segments.map((seg,si) => {
              if (seg.type==="stage") return <p key={si} style={{ fontSize:"12px", fontStyle:"italic", color:"#4A4845", lineHeight:1.7, margin:"2px 0" }}>{seg.text}</p>;
              if (seg.type==="section_break") return <div key={si} style={{ borderTop:"1px solid #222", margin:"10px 0 6px", opacity:.6 }}/>;
              if (seg.type==="coaching") return <p key={si} style={{ fontSize:"13px", lineHeight:1.85, color:"#9A9590", whiteSpace:"pre-wrap" }}>{renderMarkdown(seg.text)}</p>;
              return <p key={si} style={{ fontSize, lineHeight:1.85, color:mInRole?"#E8E5DE":"#C0BDB8", whiteSpace:"pre-wrap", borderLeft:mInRole?`2px solid ${mc.accent}35`:"none", paddingLeft:mInRole?"12px":"0" }}>{renderMarkdown(seg.text)}</p>;
            })}
          </div>
        ) : <p style={{ fontSize, lineHeight:1.85, color:"#6A6760", whiteSpace:"pre-wrap" }}>{renderMarkdown(msg.content)}</p>}
        {isA && innerText && (
          <div style={{ display:"flex", alignItems:"center", gap:"8px", marginTop:"3px" }}>
            <div style={{ width:"12px", height:"1px", background:mc.accent, opacity:.25, flexShrink:0 }}/>
            <p style={{ fontSize:"11px", fontStyle:"italic", color:mc.accent, opacity:.5 }}>{innerText}</p>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0C0C0C", color:"#F0EDE6", fontFamily:"'Inter',sans-serif", fontWeight:300, display:"flex", flexDirection:"column" }}>
      <style>{css}</style>
      {/* Header */}
      <div style={{ padding:isMobile?"12px 14px":"20px 28px", borderBottom:"1px solid #1A1A1A", display:"flex", alignItems:"center", gap:"10px", flexShrink:0 }}>
        <span style={{ fontFamily:"'Playfair Display',serif", fontSize:isMobile?"16px":"18px", fontWeight:500, letterSpacing:".04em" }}>Profess</span>
        {!isMobile && <span style={{ fontSize:"10px", color:"#3A3530", fontStyle:"italic", letterSpacing:".05em" }}>You cannot move people you do not understand.</span>}
        <div style={{ marginLeft:"auto", display:"flex", gap:"6px", alignItems:"center" }}>
          {!isMobile && <span style={{ fontSize:"10px", color:"#4A4540", letterSpacing:".08em", textTransform:"uppercase", border:"1px solid #2A2520", padding:"3px 10px" }}>{sessionMode}</span>}
          {!isMobile && intensity && (
            <span style={{ fontSize:"10px", letterSpacing:".08em", textTransform:"uppercase", padding:"3px 10px", color: intensity==="no_mercy"?"#BC5A5A": intensity==="challenging"?"#C8A870":"#7A9A70", border:`1px solid ${intensity==="no_mercy"?"#BC5A5A33": intensity==="challenging"?"#C8A87033":"#7A9A7033"}` }}>
              {intensity==="no_mercy"? (lang==="id"?"🔥 Tanpa Ampun":"🔥 No Mercy") : intensity==="challenging"? (lang==="id"?"⚡ Menantang":"⚡ Challenging") : (lang==="id"?"🌱 Nyaman":"🌱 Comfortable")}
            </span>
          )}
          <button onClick={()=>setIsMobile(p=>!p)} title="Switch layout" style={{ background:"none", border:"1px solid #1E1E1E", color:"#4A4540", fontFamily:"inherit", fontSize:"13px", padding:"4px 8px", cursor:"pointer" }}>{isMobile?"🖥":"📱"}</button>
          <button onClick={()=>{stopSpeech();setSpeechEnabled(p=>!p);}} style={{ background:"none", border:"1px solid #1E1E1E", color:speechEnabled?"#C8B89A":"#3A3A3A", fontFamily:"inherit", fontSize:"10px", padding:"4px 8px", cursor:"pointer" }}>{speechEnabled?"🔊":"🔇"}</button>
          <button onClick={resetSession} style={{ background:"none", border:"1px solid #1E1E1E", color:"#4A4540", fontFamily:"inherit", fontSize:"10px", padding:"4px 8px", cursor:"pointer", letterSpacing:".05em", textTransform:"uppercase" }}>New</button>
        </div>
      </div>
      ...
      {isMobile ? (
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
          {/* Mobile char strip */}
          <div style={{ background:isTransitioning?"#0C0C0C":charMeta.bg, transition:"background .4s", borderBottom:"1px solid #161616", padding:"8px 14px", display:"flex", alignItems:"center", gap:"12px", flexShrink:0 }}>
            {charAnimBlock("64px")}
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"13px", color:charMeta.accent }}>{charMeta.name}</div>
              <div style={{ fontSize:"9px", color:"#3A3530", letterSpacing:".07em", textTransform:"uppercase" }}>{charMeta.title}</div>
              {currentMood !== "neutral" && <div style={{ marginTop:"3px", fontSize:"8px", color:charMeta.accent, background:`${charMeta.accent}20`, padding:"2px 7px", display:"inline-block", borderRadius:"2px", letterSpacing:".08em", textTransform:"uppercase" }}>{currentMood}</div>}
            </div>
            {isSpeaking && <div style={{ display:"flex", gap:"2px", flexShrink:0 }}>{[0,.12,.24,.12,0].map((d,i)=><div key={i} style={{ width:"2px", height:`${5+i*2}px`, background:charMeta.accent, borderRadius:"1px", animation:`waveBar .6s ease-in-out ${d}s infinite` }}/>)}</div>}
          </div>

          {/* Mobile chat */}
          <div style={{ flex:1, overflowY:"auto", padding:"14px 14px 8px", display:"flex", flexDirection:"column", gap:"18px" }}>
            {msgList("15px")}
            {loading && (() => {
              const coachSVG = buildSVG(CHARS.default, "thinking", false);
              return (
                <div style={{ display:"flex", flexDirection:"column", gap:"4px" }}>
                  <span style={{ fontSize:"9px", letterSpacing:".1em", textTransform:"uppercase", fontWeight:500, color:charMeta.accent }}>{isInRole?charMeta.name:"Profess"}</span>
                  <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
                    <div style={{ width:"28px", height:"35px", animation:"moodThinking 2.5s ease-in-out infinite", flexShrink:0 }} dangerouslySetInnerHTML={{ __html:coachSVG }}/>
                    <div style={{ display:"flex", gap:"5px" }}>{[0,.2,.4].map((d,i)=><div key={i} style={{ width:"5px", height:"5px", background:charMeta.accent, borderRadius:"50%", opacity:.4, animation:`pulse 1.2s ease-in-out ${d}s infinite` }}/>)}</div>
                  </div>
                </div>
              );
            })()}
          ...
          {/* Mobile input */}
          <div style={{ padding:"8px 12px 14px", borderTop:"1px solid #161616", background:"#0C0C0C", display:"flex", flexDirection:"column", gap:"7px" }}>
            <div style={{ display:"flex", gap:"6px" }}>
              {lastExchange && <button onClick={tryAgain} style={{ background:"none", border:`1px solid ${charMeta.accent}50`, color:charMeta.accent, fontFamily:"inherit", fontSize:"11px", padding:"6px 12px", cursor:"pointer", letterSpacing:".05em", textTransform:"uppercase" }}>↩ {lang==="id"?"Coba lagi":"Try again"}</button>}
              <button onClick={endSession} style={{ background:"none", border:"1px solid #1E1E1E", color:"#3A3530", fontFamily:"inherit", fontSize:"11px", padding:"6px 12px", cursor:"pointer", marginLeft:"auto", letterSpacing:".05em", textTransform:"uppercase" }}>{lang==="id"?"Akhiri":"End"}</button>
            </div>
            {micError && <p style={{ fontSize:"11px", color:"#8B4040", textAlign:"center" }}>{micError}</p>}
            <div style={{ display:"flex", gap:"8px", alignItems:"flex-end" }}>
              <textarea ref={textareaRef} style={{ flex:1, background:"#141414", border:`1px solid ${isListening?"#BC7A7A":"#222"}`, color:"#F0EDE6", fontFamily:"inherit", fontSize:"16px", fontWeight:300, lineHeight:1.6, padding:"12px 14px", resize:"none", outline:"none", minHeight:"54px", maxHeight:"160px", overflowY:"auto" }}
                placeholder={isListening?"Listening...":lang==="id"?"Ketik sesuatu...":"Say something..."} value={input} onChange={handleTA} onKeyDown={handleKeyDown} rows={1}/>
              {isSpeaking && <button onClick={stopSpeech} style={{ background:"#180A0A", border:"1px solid #BC7A7A", color:"#BC7A7A", width:"54px", height:"54px", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", fontSize:"18px", flexShrink:0 }}>⏹</button>}
              <button onClick={toggleMic} className={isListening?"mic-active":""} style={{ background:isListening?"#2A1414":"none", border:`1px solid ${isListening?"#BC7A7A":"#333"}`, color:isListening?"#BC7A7A":"#5A5550", width:"54px", height:"54px", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", fontSize:"20px", flexShrink:0 }}>{isListening?"⏹":"🎙"}</button>
              <button id="psend" onClick={sendMessage} disabled={loading||!input.trim()} style={{ background:"none", border:`1px solid ${input.trim()&&!loading?charMeta.accent:"#222"}`, color:input.trim()&&!loading?charMeta.accent:"#333", fontFamily:"inherit", fontSize:"12px", fontWeight:500, letterSpacing:".06em", padding:"0 16px", cursor:loading||!input.trim()?"not-allowed":"pointer", textTransform:"uppercase", height:"54px", flexShrink:0, opacity:loading||!input.trim()?.4:1 }}>{lang==="id"?"Kirim":"Send"}</button>
            </div>
            {isListening && <div style={{ display:"flex", alignItems:"center", gap:"6px" }}>
              <div style={{ display:"flex", gap:"2px" }}>{[0,.08,.16,.24,.32].map((d,i)=><div key={i} style={{ width:"2px", height:`${5+i*3}px`, background:"#BC7A7A", borderRadius:"1px", animation:`waveBar .5s ease-in-out ${d}s infinite` }}/>)}</div>
              <span style={{ fontSize:"10px", color:"#BC7A7A", letterSpacing:".06em", textTransform:"uppercase" }}>{lang==="id"?"Rekam — jeda untuk kirim":"Recording — pause to send"}</span>
            </div>}
          </div>
        </div>
      ) : (
        ...
        <div style={{ flex:1, display:"flex", overflow:"hidden" }}>
          <div style={{ width:"200px", flexShrink:0, borderRight:"1px solid #161616", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:isTransitioning?"#0C0C0C":charMeta.bg, transition:"background .4s ease", position:"fixed", top:"61px", left:0, height:"calc(100vh - 61px)", overflow:"hidden", zIndex:10 }}>
            <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"12px", padding:"20px 12px" }}>
              ...
              <div style={{ textAlign:"center", opacity:isTransitioning?0:1, transition:"opacity .38s" }}>
                ...
                {currentMood !== "neutral" && <div style={{ marginTop:"6px", fontSize:"8px", color:charMeta.accent, letterSpacing:".1em", textTransform:"uppercase", background:`${charMeta.accent}20`, padding:"3px 9px", display:"inline-block", borderRadius:"2px" }}>{currentMood}</div>}
              </div>
              {isSpeaking && <div style={{ display:"flex", gap:"3px", alignItems:"center" }}>{[0,.12,.24,.12,0].map((d,i)=><div key={i} style={{ width:"2px", height:`${8+i*3}px`, background:charMeta.accent, borderRadius:"1px", animation:`waveBar .6s ease-in-out ${d}s infinite`, opacity:.8 }}/>)}</div>}
            </div>
          </div>
          <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", marginLeft:"200px" }}>
              ...
              return (
                <div style={{ display:"flex", flexDirection:"column", gap:"5px" }}>
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                    ...
                    <div style={{ display:"flex", gap:"5px", padding:"4px 0" }}>{[0,.2,.4].map((d,i)=><div key={i} style={{ width:"5px", height:"5px", background:charMeta.accent, borderRadius:"50%", opacity:.4, animation:`pulse 1.2s ease-in-out ${d}s infinite` }}/>)}</div>
                  </div>
                </div>
              );
            })()}
            ...
            <div style={{ padding:"12px 24px 20px", borderTop:"1px solid #161616", display:"flex", flexDirection:"column", gap:"7px" }}>
              <div style={{ display:"flex", gap:"8px", marginBottom:"2px" }}>
                {lastExchange && <button onClick={tryAgain} style={{ background:"none", border:"1px solid #2A2520", color:"#6A6560", fontFamily:"inherit", fontSize:"10px", padding:"4px 12px", cursor:"pointer", letterSpacing:".08em", textTransform:"uppercase", transition:"all .2s" }} onMouseEnter={e=>{e.currentTarget.style.borderColor=charMeta.accent;e.currentTarget.style.color=charMeta.accent;}} onMouseLeave={e=>{e.currentTarget.style.borderColor="#2A2520";e.currentTarget.style.color="#6A6560";}}>↩ {lang==="id"?"Coba lagi":"Try again"}</button>}
                <button onClick={endSession} style={{ background:"none", border:"1px solid #1E1E1E", color:"#3A3530", fontFamily:"inherit", fontSize:"10px", padding:"4px 12px", cursor:"pointer", letterSpacing:".08em", textTransform:"uppercase", transition:"all .2s", marginLeft:"auto" }} onMouseEnter={e=>{e.currentTarget.style.borderColor="#3A3530";e.currentTarget.style.color="#6A6560";}} onMouseLeave={e=>{e.currentTarget.style.borderColor="#1E1E1E";e.currentTarget.style.color="#3A3530";}}>{lang==="id"?"Akhiri sesi":"End session"}</button>
              </div>
              {micError && <p style={{ fontSize:"11px", color:"#8B4040", textAlign:"center" }}>{micError}</p>}
              <div style={{ display:"flex", gap:"8px", alignItems:"flex-end" }}>
                <textarea ref={textareaRef} style={{ flex:1, background:"#111", border:`1px solid ${isListening?"#BC7A7A":"#1A1A1A"}`, color:"#F0EDE6", fontFamily:"inherit", fontSize:"14px", fontWeight:300, lineHeight:1.6, padding:"10px 13px", resize:"none", outline:"none", minHeight:"46px", maxHeight:"140px", overflowY:"auto", transition:"border-color .2s" }} placeholder={isListening?"Listening...":"Say something..."} value={input} onChange={handleTA} onKeyDown={handleKeyDown} rows={1} onFocus={e=>{if(!isListening)e.target.style.borderColor="#242424";}} onBlur={e=>{if(!isListening)e.target.style.borderColor="#1A1A1A";}}/>
                {isSpeaking && <button onClick={stopSpeech} style={{ background:"#180A0A", border:"1px solid #BC7A7A", color:"#BC7A7A", fontSize:"16px", padding:0, cursor:"pointer", width:"46px", height:"46px", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>⏹</button>}
                <button onClick={toggleMic} className={isListening?"mic-active":""} style={{ background:isListening?"#2A1414":"none", border:`1px solid ${isListening?"#BC7A7A":"#242424"}`, color:isListening?"#BC7A7A":"#4A4540", fontSize:"18px", padding:0, cursor:"pointer", width:"46px", height:"46px", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>{isListening?"⏹":"🎙"}</button>
                <button id="psend" onClick={sendMessage} disabled={loading||!input.trim()} style={{ background:"none", border:`1px solid ${input.trim()&&!loading?charMeta.accent:"#222"}`, color:input.trim()&&!loading?charMeta.accent:"#3A3A3A", fontFamily:"inherit", fontSize:"11px", fontWeight:500, letterSpacing:".08em", padding:"10px 18px", cursor:loading||!input.trim()?"not-allowed":"pointer", textTransform:"uppercase", height:"46px", transition:"all .2s", opacity:loading||!input.trim()?.4:1 }}>Send</button>
              </div>
              {isListening && <div style={{ display:"flex", alignItems:"center", gap:"7px" }}>
                <div style={{ display:"flex", gap:"2px" }}>{[0,.08,.16,.24,.32].map((d,i)=><div key={i} style={{ width:"2px", height:`${5+i*3}px`, background:"#BC7A7A", borderRadius:"1px", animation:`waveBar .5s ease-in-out ${d}s infinite`, opacity:.8 }}/>)}</div>

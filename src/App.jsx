...
export default function Profess() {
  ...
  if (screen === "landing") {
    const heroCharObjs = [
      { obj: CHARS.default, role: "default" },
      { obj: generateChar("journalist"), role: "journalist" },
      { obj: generateChar("friend_female"), role: "friend_female" },
      { obj: generateChar("judge"), role: "judge" },
    ];
    const heroChars = heroCharObjs.map(({ obj, role }) => ({ svg: buildSVG(obj, "neutral", false), name: obj.name || "Coach", role }));
    const heroRoleDesc = r => ({ interviewer:"Formal — job interviews, performance reviews", reviewer:"Formal — job interviews, performance reviews", auditor:"Formal — job interviews, performance reviews", examiner:"Formal — thesis defense, academic evaluation", professor_academic:"Formal — thesis defense, academic evaluation", dean:"Formal — thesis defense, academic evaluation", journalist:"Formal — press conferences, media training", critic:"Formal — press conferences, media training", judge:"Formal — mock trials, debates", prosecutor:"Formal — mock trials, debates", friend_female:"Social — reconnecting, casual conversation", friend_male:"Social — reconnecting, casual conversation", best_friend:"Social — reconnecting, casual conversation", colleague:"Social — workplace dynamics", manager:"Social — workplace dynamics", crush:"Social — romantic situations", romantic_interest:"Social — romantic situations", date:"Social — romantic situations", stranger:"Social — first impressions", new_acquaintance:"Social — first impressions", negotiator:"Formal — pitching, negotiation", ceo:"Formal — pitching, negotiation", executive:"Formal — pitching, negotiation", default:"Available in all sessions" }[r] || "Available in all sessions");
    const lrMaleObj = generateChar("friend_male");
    const lrFemaleObj = generateChar("friend_female");
    const livingRoomMale = buildSVG(lrMaleObj, roomMood, false);
    const livingRoomFemale = buildSVG(lrFemaleObj, roomMood, false);
    const aboutRole = pick(["interviewer","journalist","colleague","friend_female"]);
    const panelCharAbout = buildSVG(generateChar(aboutRole), "neutral", false);
    ...
    const panelCharTerms = buildSVG(judgeChar, "serious", false);
    const panelCharBegin = buildSVG(CHARS.default, "neutral", false);
    ...
    return (
      <div className="screen-enter" style={{ ...BASE, overflowY:"auto", filter:warmMode?"sepia(0.18) saturate(1.2) brightness(1.02)":"none", transition:"filter 1.2s ease" }}>
        ...
        <div style={{ position:"relative", height:"100vh", display:"flex", flexDirection:"column", overflow:"hidden", paddingBottom:"0" }}>
          ...
          {/* Character row — bottom of hero */}
          <div style={{ display:"flex", justifyContent:"center", alignItems:"flex-end", gap:"clamp(16px,4vw,56px)", padding:"60px 40px 24px", position:"relative", zIndex:2, flexShrink:0 }}>
            {heroChars.map(({ svg, name, role }, i) => {
              const heroAnims = ["coachNod 4s ease-in-out infinite","journalistSway 3s ease-in-out infinite","friendBob 2.8s ease-in-out infinite","judgeTap 5s ease-in-out infinite"];
              return (
                <div key={i} style={{ position:"relative", flexShrink:0 }}
                  onMouseEnter={e=>e.currentTarget.querySelector('.hero-tt').style.opacity="1"}
                  onMouseLeave={e=>e.currentTarget.querySelector('.hero-tt').style.opacity="0"}>
                  {/* Tooltip */}
                  <div className="hero-tt" style={{ position:"absolute", bottom:"100%", left:"50%", transform:"translateX(-50%)", marginBottom:"8px", background:"#0E0E0E", border:"1px solid #2A2520", padding:"10px 14px", opacity:0, transition:"opacity 0.2s ease", pointerEvents:"none", zIndex:100, whiteSpace:"nowrap", textAlign:"center" }}>
                    <p style={{ fontFamily:"'Playfair Display',serif", fontSize:"13px", color:"#C8A870", marginBottom:"3px" }}>{name}</p>
                    <p style={{ fontSize:"11px", color:"#5A5550", fontWeight:300 }}>{heroRoleDesc(role)}</p>
                  </div>
                  <div style={{ width:"90px", height:"112px", opacity:.10, animation:heroAnims[i] }} dangerouslySetInnerHTML={{ __html: svg }}/>
                </div>
              );
            })}
          </div>

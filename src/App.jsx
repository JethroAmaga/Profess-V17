...
export default function Profess() {
  ...
  // ── LANDING ────────────────────────────────────────────────────────────────
  if (screen === "landing") {
    const allRoles = Object.keys(ROLE_TITLES);
    const coachSVG   = buildSVG(CHARS.default, "neutral", false);
    const char1SVG   = buildSVG(generateChar(pick(allRoles)), "neutral", false);
    const char2SVG   = buildSVG(generateChar(pick(allRoles)), "neutral", false);
    const char3SVG   = buildSVG(generateChar(pick(allRoles)), "neutral", false);
    return (
      <div className="screen-enter" style={{ ...BASE, height:"100vh", overflow:"hidden", position:"relative" }}>
        <style>{css}</style>
        {/* Grain texture */}
        ...
        <nav style={{ position:"absolute", top:0, left:0, right:0, padding:"24px 40px", display:"flex", alignItems:"center", zIndex:10 }}>
          ...
          <div style={{ marginLeft:"auto", display:"flex", gap:"32px" }}>
            <button onClick={() => setScreen("about")} style={{ background:"none", border:"none", fontSize:"11px", color:"#3A3530", textDecoration:"none", letterSpacing:".1em", textTransform:"uppercase", transition:"color .2s", padding:0, cursor:"pointer" }}
              onMouseEnter={e=>e.currentTarget.style.color="#9A9590"} onMouseLeave={e=>e.currentTarget.style.color="#3A3530"}>About</button>
            <button onClick={() => setScreen("terms")} style={{ background:"none", border:"none", fontSize:"11px", color:"#3A3530", textDecoration:"none", letterSpacing:".1em", textTransform:"uppercase", transition:"color .2s", padding:0, cursor:"pointer" }}
              onMouseEnter={e=>e.currentTarget.style.color="#9A9590"} onMouseLeave={e=>e.currentTarget.style.color="#3A3530"}>Terms</button>
          </div>
        </nav>

        {/* Character silhouettes — background layer */}
        {/* Coach — primary, far right center */}
        <div style={{ position:"absolute", right:"-2vw", bottom:"0", width:"30vh", height:"38vh", opacity:.14, pointerEvents:"none", zIndex:1 }}
          dangerouslySetInnerHTML={{ __html: coachSVG }}/>
        {/* Char 1 — far right, bottom 10% */}
        <div style={{ position:"absolute", right:"2vw", bottom:"10%", width:"180px", opacity:.07, pointerEvents:"none", zIndex:1, transform:"rotate(-3deg)" }}
          dangerouslySetInnerHTML={{ __html: char1SVG }}/>
        {/* Char 2 — left side, 60% from top */}
        <div style={{ position:"absolute", left:"2vw", top:"60%", width:"120px", opacity:.05, pointerEvents:"none", zIndex:1, transform:"rotate(2deg)" }}
          dangerouslySetInnerHTML={{ __html: char2SVG }}/>
        {/* Char 3 — right side, 15% from top */}
        <div style={{ position:"absolute", right:"18vw", top:"15%", width:"100px", opacity:.04, pointerEvents:"none", zIndex:1 }}
          dangerouslySetInnerHTML={{ __html: char3SVG }}/>

        {/* Hero */}
        <div style={{ position:"relative", zIndex:2, height:"100vh", display:"flex", flexDirection:"column", alignItems:"flex-start", justifyContent:"center", padding:"0 40px 0 clamp(40px,8vw,140px)" }}>
          <div style={{ maxWidth:"640px", width:"100%", display:"flex", flexDirection:"column", gap:"24px" }}>
            {/* Tagline with label above first line */}
            <div>
          ...
          {/* Scroll indicator */}
          <div style={{ position:"absolute", bottom:"48px", left:"50%", transform:"translateX(-50%)", display:"flex", flexDirection:"column", alignItems:"center", gap:"8px", opacity:.25 }}>
  ...
  // ── ABOUT ──────────────────────────────────────────────────────────────────
  if (screen === "about") {
    const aboutCoachSVG = buildSVG(CHARS.default, "warm", false);
    return (
    <div className="screen-enter" style={{ ...BASE, display:"flex", flexDirection:"column" }}>
      <style>{css}</style>
      <div className="grain-layer"/>
      {/* Coach character — right side, vertically centered */}
      <div style={{ position:"fixed", right:"4vw", top:"50%", transform:"translateY(-50%)", width:"160px", opacity:.12, pointerEvents:"none", zIndex:0 }}
        dangerouslySetInnerHTML={{ __html: aboutCoachSVG }}/>
      ...
      <div style={{ position:"relative", zIndex:1, flex:1, display:"flex", justifyContent:"center", padding:"72px 40px 80px", overflowY:"auto" }}>
        <div style={{ maxWidth:"680px", width:"100%" }}>
          <p style={{ fontSize:"9px", letterSpacing:".2em", textTransform:"uppercase", color:"#3A3530", marginBottom:"16px" }}>About</p>
          <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(32px,4vw,48px)", fontWeight:400, color:"#F0EDE6", lineHeight:1.2, marginBottom:"48px" }}>About Profess</h2>
          <div style={{ display:"flex", flexDirection:"column", gap:"24px", maxWidth:"560px" }}>
            <p style={{ fontSize:"15px", lineHeight:1.9, color:"#D8D5CE" }}>
              Profess was built on the belief that communication is not a talent — it is a skill. And like any skill, it improves through deliberate practice against real resistance. Not a script. Not a mirror. A character that responds the way your audience actually will.
            </p>
            <p style={{ fontSize:"15px", lineHeight:1.9, color:"#7A7570" }}>
              Most practice tools give you a passive audience. Profess gives you an active one — a character that pushes back, reacts, and responds the way your real audience will. The coaching comes after the exchange, not during it.
            </p>
            <p style={{ fontSize:"15px", lineHeight:1.9, color:"#7A7570" }}>
              You cannot prepare for a conversation without experiencing the other person's perspective. Profess is built on that single principle. The session ends when you're ready — not when the tool runs out of things to say.
            </p>
          </div>
          <div style={{ marginTop:"64px", paddingTop:"40px", borderTop:"1px solid #141414" }}>
  ...
  // ── TERMS ──────────────────────────────────────────────────────────────────
  if (screen === "terms") {
    const judgeChar = generateChar("judge");
    const judgeSVG = buildSVG(judgeChar, "serious", false);
    return (
    <div className="screen-enter" style={{ ...BASE, display:"flex", flexDirection:"column" }}>
      <style>{css}</style>
      <div className="grain-layer"/>
      {/* Judge character — right side, vertically centered */}
      <div style={{ position:"fixed", right:"4vw", top:"50%", transform:"translateY(-50%)", width:"160px", opacity:.10, pointerEvents:"none", zIndex:0, display:"flex", flexDirection:"column", alignItems:"center", gap:"6px" }}>
        <div dangerouslySetInnerHTML={{ __html: judgeSVG }}/>
        <p style={{ fontSize:"8px", letterSpacing:".14em", textTransform:"uppercase", color:"#C8A870", textAlign:"center" }}>Terms & Conditions</p>
      </div>
      <div style={{ position:"relative", zIndex:1, padding:"0 40px", height:"56px", borderBottom:"1px solid #141414", display:"flex", alignItems:"center" }}>
        <button onClick={() => setScreen("landing")} style={{ background:"none", border:"none", color:"#3A3530", fontSize:"10px", letterSpacing:".1em", textTransform:"uppercase", display:"flex", alignItems:"center", gap:"5px", transition:"color .2s", padding:0, cursor:"pointer" }}
          onMouseEnter={e=>e.currentTarget.style.color="#6A6560"} onMouseLeave={e=>e.currentTarget.style.color="#3A3530"}>
      ...
      <div style={{ position:"relative", zIndex:1, flex:1, display:"flex", justifyContent:"center", padding:"72px 40px 80px", overflowY:"auto" }}>
        <div style={{ maxWidth:"680px", width:"100%" }}>
          <p style={{ fontSize:"9px", letterSpacing:".2em", textTransform:"uppercase", color:"#3A3530", marginBottom:"16px" }}>Legal</p>
          <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(32px,4vw,48px)", fontWeight:400, color:"#F0EDE6", lineHeight:1.2, marginBottom:"48px" }}>Terms & Conditions</h2>
          <div style={{ display:"flex", flexDirection:"column", gap:"0", maxWidth:"560px" }}>
            {[
              ...
              { title:"User conduct", body:"Profess will not engage with requests to portray religious figures, convicted criminals, terrorists, historical dictators, or real public figures in roleplay. Sessions involving abusive, offensive, or sexually explicit content will be paused. Users who continue such conduct may be blocked from the service." },
              { title:"Data and privacy", body:"Conversation content is processed to generate AI responses and is not stored beyond the active session. No personally identifiable information is retained after the session ends. Session data is not used to train AI models without explicit consent." },
              ...
              <div key={i} style={{ padding:"24px 0", borderBottom:"1px solid #0E0E0E" }}>
                <p style={{ fontSize:"13px", fontWeight:500, color:"#8A8580", marginBottom:"10px", letterSpacing:".04em" }}>{item.title}</p>
                <p style={{ fontSize:"13px", lineHeight:1.9, color:"#4A4540" }}>{item.body}</p>
              </div>
            ))}
          </div>
          <div style={{ marginTop:"64px", paddingTop:"40px", borderTop:"1px solid #141414" }}>

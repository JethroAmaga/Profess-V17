...
// ─── Cool accent palette (replaces the brass/amber family) ─────────────────
const ACCENT = { formal: "#5B8AA6", social: "#A9577B" };
const INK = "#C7CDD6"; // cool muted body text, replaces #C8A458
const BG = "#070809"; // cool near-black
...
const MOOD_GLOW = {
  neutral: null, surprised: "#A8B8D0", amused: ACCENT.formal, thinking: "#7A8AA0",
  skeptical: "#8A7A60", serious: "#4A4040", uncomfortable: "#8A6060", listening: "#8AA08A", warm: ACCENT.social,
};
...
::-webkit-scrollbar-thumb { background:#1C1E22; border-radius:3px; }
...
export default function Profess() {
  ...
  const resetSession = () => {
    ...
    setStage("lang"); setLang(null); setSessionMode(null); setPendingMode(null); setIntensity(null); setScenario(null);
  ...
  function msgList() {
    return messages.map((m, i) => {
      ...
      const speakerColor = m.role === "user" ? "#7C828C" : (m.inRole ? sessionAccent : INK);
      return (
  ...
  function Overlay({ children, width = "min(520px, 90vw)", align = "left" }) {
    return (
      <motion.div
        initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 12 }}
        transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }}
        style={{ position: "absolute", left: isMobile ? "50%" : "9%", top: "50%", transform: isMobile ? "translate(-50%,-50%)" : "translateY(-50%)",
          width, zIndex: 30, maxHeight: "82vh", overflowY: "auto", textAlign: align, paddingTop: "10px", paddingBottom: "16px",
    ...
    return (
      <div style={{ display: "flex", gap: "5px", marginBottom: "22px" }}>
        {ONBOARDING_ORDER.map((s, idx) => (
          <div key={s} style={{ height: "3px", borderRadius: "2px", width: idx === i ? "20px" : "8px",
            background: idx === i ? sessionAccent : "#222428", transition: "all .3s" }} />
        ))}
      </div>
    );
  };
  ...
  const backBtn = (to) => (
    <button onClick={() => setStage(to)} style={{ background: "none", border: "none", color: "#7C828C", fontSize: "10px", letterSpacing: ".08em", textTransform: "uppercase", display: "flex", alignItems: "center", gap: "5px", padding: 0, marginBottom: "18px" }}>
      <IconArrowLeft /> Back
    </button>
  );
  ...
  const ChoiceRow = ({ onClick, accent, children, sub }) => (
    <button onClick={onClick} onMouseEnter={(e)=>e.currentTarget.style.setProperty("--c","1")} onMouseLeave={(e)=>e.currentTarget.style.setProperty("--c","0")}
      ...
      {sub && <span style={{ color: "#7C828C", fontSize: "12px", display: "block", marginTop: "2px" }}>{sub}</span>}
    </button>
  );
  ...
  function StageOverlay() {
    if (stage === "intro") return (
      <Overlay>
        <h1 style={{ fontFamily: "'Playfair Display',serif", fontStyle: "italic", fontWeight: 400, fontSize: "clamp(32px,5vw,46px)", color: "#F0EDE6", marginBottom: "14px", letterSpacing: "-0.02em" }}>Profess</h1>
        <p style={{ color: INK, fontSize: "14px", lineHeight: 1.8, marginBottom: "26px", maxWidth: "38ch" }}>
          Practice the conversation before it happens. A presence that reacts to you, presses back, and tells you what it really felt.
        </p>
        <button onClick={() => setStage("lang")} style={{ background: "none", border: "none", color: sessionAccent, fontSize: "15px",
    ...
    if (stage === "disclaimer") {
      ...
      return (
        <Overlay>
          ...
          <button onClick={() => setStage("intensity")} style={{ background: "none", border: "none", color: sessionAccent, fontSize: "13px", padding: 0, borderBottom: `1px solid ${sessionAccent}55` }}>
            {c.cta} →
          </button>
        </Overlay>
      );
    ...
    if (stage === "intensity") return (
      <Overlay>
        ...
        {INTENSITY_LEVELS[lang || "en"].map(lvl => (
          <ChoiceRow key={lvl.key} accent={lvl.color} sub={lvl.desc} onClick={() => { setIntensity(lvl.key); setStage("scenario"); }}>
            {lvl.label}
          </ChoiceRow>
        ))}
      </Overlay>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
            {groups.map((g, gi) => (
              <div key={gi}>
                ...
                {g.items.map((item, ii) => (
                  <ChoiceRow key={ii} onClick={() => { setScenario(item); startSession(pendingMode, item); }}>{item}</ChoiceRow>
                ))}
              </div>
            ))}
          </div>
    ...
    if (stage === "summary") return (
      <Overlay>
        ...
        <p style={{ fontSize: "13px", lineHeight: 1.8, color: INK, whiteSpace: "pre-wrap", marginBottom: "24px" }}>{summary}</p>
        <button onClick={resetSession} style={{ background: "none", border: "none", color: sessionAccent, fontSize: "13px", padding: 0, borderBottom: `1px solid ${sessionAccent}55` }}>
  ...
  return (
    <div style={{ minHeight: "100vh", height: "100vh", overflow: "hidden", background: BG, color: "#F0EDE6", fontFamily: "'Inter',sans-serif", fontWeight: 300, position: "relative" }}>
      ...
      {/* Persistent ambient light field — encodes warmth/lean/mood, never literal furniture */}
      <motion.div style={{ position: "absolute", inset: 0, pointerEvents: "none", zIndex: 0,
        background: `radial-gradient(circle at 78% 62%, ${glowColor || sessionAccent}1A 0%, transparent 55%)` }}
      ...
      <motion.div
        ...
        {inConversation && (
          <div style={{ position: "absolute", bottom: "8%", left: "50%", transform: "translateX(-50%)", textAlign: "center", whiteSpace: "nowrap" }}>
            ...
            <p style={{ fontSize: "11px", color: "#7C828C", marginTop: "2px" }}>{charMeta.title}</p>
      ...
      {inConversation && (
        <>
          <div style={{ position: "absolute", top: 0, left: 0, right: 0, padding: "20px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", zIndex: 10 }}>
            <button onClick={() => setStage("intro")} style={{ background: "none", border: "none", color: "#7C828C", fontSize: "13px", fontFamily: "'Inter',sans-serif" }}>Profess</button>
            <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
              <button onClick={() => setTranscriptOpen(o => !o)} style={{ background: "none", border: "1px solid #1C1E22", borderRadius: "8px", color: "#9CA0A8", fontSize: "11px", padding: "6px 10px" }}>
                {transcriptOpen ? (lang === "id" ? "Sembunyikan" : "Hide transcript") : (lang === "id" ? "Transkrip" : "Transcript")}
              </button>
              <button onClick={() => setSpeechEnabled(s => !s)} style={{ background: "none", border: "none", color: "#9CA0A8" }}>{speechEnabled ? <IconVolume/> : <IconMute/>}</button>
              <button onClick={resetSession} style={{ background: "none", border: "none", color: "#9CA0A8" }}><IconRefresh/></button>
            </div>
          </div>
          ...
          <AnimatePresence>
            {transcriptOpen && (
              <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }}
                ...
                {msgList()}
                {loading && <p style={{ fontSize: "12px", color: "#7C828C" }}>{lang === "id" ? "..." : "thinking..."}</p>}
          ...
          <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, padding: "16px 28px 22px", zIndex: 10, background: "linear-gradient(to top, rgba(7,8,9,0.92), transparent)" }}>
            <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px", marginBottom: "8px" }}>
              {lastExchange && <button onClick={tryAgain} style={{ background: "none", border: "none", color: "#7C828C", fontSize: "11px" }}>{lang === "id" ? "Coba lagi" : "Try again"}</button>}
              <button onClick={endSession} style={{ background: "none", border: "none", color: "#7C828C", fontSize: "11px" }}>{lang === "id" ? "Akhiri" : "End session"}</button>
            </div>
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "flex-end" }}>
              <textarea ref={textareaRef} value={input} onChange={handleTA} onKeyDown={handleKeyDown}
                ...
                style={{ flex: 1, minHeight: "48px", maxHeight: "160px", resize: "none", background: "#101114",
                  border: isListening ? "1px solid #7A4040" : "1px solid #1C1E22", borderRadius: "12px", color: "#F0EDE6",
                  padding: "13px 14px", fontFamily: "inherit", fontSize: "14px" }} />
              {isSpeaking && <button onClick={stopSpeech} style={{ background: "#101114", border: "1px solid #1C1E22", borderRadius: "10px", padding: "12px", color: "#9CA0A8" }}><IconStop/></button>}
              <button onClick={toggleMic} className={isListening ? "mic-active" : ""} style={{ background: "#101114", border: "1px solid #1C1E22", borderRadius: "10px", padding: "12px", color: isListening ? "#BC5A5A" : "#9CA0A8" }}><IconMic/></button>
              <button id="psend" onClick={sendMessage} disabled={loading || !input.trim()} style={{ background: sessionAccent, border: "none", borderRadius: "10px", padding: "12px 16px", color: "#0A0B0D", opacity: loading || !input.trim() ? .4 : 1 }}><IconSend/></button>
            </div>
          </div>
        </>
      )}

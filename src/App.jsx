...
const ACCENT = { formal: "#C7A47E", social: "#CC9C96" };
...
export default function Profess() {
  ...
  const startSession = async (mode, selectedScenario = null, groupIdx = sceneGroupIdx) => {
    // Re-roll the backdrop pick for this scene every time a session starts —
    // otherwise the cached pick from the first visit sticks for the rest of
    // the app's lifetime, so picking the same scenario again always shows
    // the same image instead of varying like a fresh visit. `groupIdx` is
    // taken as a direct argument (not the sceneGroupIdx state) because the
    // caller's setSceneGroupIdx() hasn't flushed yet when this runs.
    const sceneForReroll = getScene(mode, groupIdx);
    if (sceneForReroll?.id) delete backdropPicksRef.current[sceneForReroll.id];
    ...
    return messages.map((m, i) => {
      ...
      return (
        <div key={i} className="msg-enter" style={{ marginBottom: "22px" }}>
          <p style={{ fontSize: "10px", letterSpacing: ".08em", textTransform: "uppercase", color: "#000", marginBottom: "6px", opacity: .8 }}>{speakerLabel}</p>
          {segs.map((seg, j) => {
            if (seg.type === "section_break") return <div key={j} style={{ borderTop: "1px solid #D8D4CC", margin: "10px 0", paddingTop: "8px" }}><span style={{ fontSize: "9px", letterSpacing: ".1em", color: "#000" }}>COACH</span></div>;
            if (seg.type === "stage") return <p key={j} style={{ fontStyle: "italic", color: "#000", fontSize: "13px", margin: "4px 0" }}>{seg.text}</p>;
            if (seg.type === "coaching") return <p key={j} style={{ fontStyle: "italic", color: "#000", fontSize: "13px", margin: "4px 0", paddingLeft: "10px" }}>{seg.text}</p>;
            return <p key={j} style={{ color: "#000", fontSize: "14px", lineHeight: 1.7, margin: "4px 0", paddingLeft: m.inRole ? "10px" : 0, borderLeft: m.inRole ? "2px solid #D8D4CC" : "none" }}>{seg.text}</p>;
          })}
          {m.inner && <p style={{ fontSize: "12px", color: "#000", opacity: .65, marginTop: "4px", paddingLeft: "10px", borderLeft: "1px solid #D8D4CC" }}>{m.inner}</p>}
        </div>
      );
    });
  }
  ...
  // Tilt wrapper — gives a panel a subtle 3D "floating" feel on pointer
  // move (desktop only; touch devices have no hover to drive it). Purely
  // a transform layer: callers own background/size/padding entirely, so
  // visuals are unaffected — only the perspective tilt is added.
  function TiltBox({ children, style }) {
    const ref = useRef(null);
    // Desktop: tilt follows the mouse position relative to this box.
    const handleMove = (e) => {
      const el = ref.current; if (!el) return;
      const { left, top, width, height } = el.getBoundingClientRect();
      const rx = (e.clientX - left - width / 2) / 25;
      const ry = (e.clientY - top - height / 2) / 25;
      el.style.transform = `rotateY(${rx}deg) rotateX(${-ry}deg)`;
    };
    const handleLeave = () => { if (ref.current) ref.current.style.transform = "rotateY(0deg) rotateX(0deg)"; };
    // Mobile: no hover, so the same tilt is driven by the device's gyroscope
    // instead — gently rocking with how the phone itself is held/tilted.
    useEffect(() => {
      if (!isMobile) return;
      const onTilt = (e) => {
        const el = ref.current; if (!el) return;
        ...
        el.style.transform = `rotateY(${gamma / 6}deg) rotateX(${-beta / 10}deg)`;
      };
      window.addEventListener("deviceorientation", onTilt);
    ...
    return (
      <div style={{ perspective: "1000px" }}>
        <div ref={ref} onMouseMove={!isMobile ? handleMove : undefined} onMouseLeave={!isMobile ? handleLeave : undefined}
          style={{ ...style, transformStyle: "preserve-3d", transition: "transform .2s ease-linear" }}>
          {children}
        </div>
      </div>
    );
  ...
  function Overlay({ children, footer, width = "min(520px, 90vw)", align = "left", placement = "left", panel = false }) {
    ...
    const topCenter = placement === "top-center" && !isMobile;
    // Onboarding panel (lang/mode/scenario/warning/session) sits between the
    // backdrop and the text as a rounded card so copy always reads against a
    // flat, legible fill instead of whatever's busy in the photo behind it.
    // Before an actual session starts the panel always matches the app's
    // base background — it must not preview the social/formal accent color
    // just because a mode was picked on an earlier screen. Only the live
    // session's transcript panel (a separate element) switches color.
    const panelBg = "#FFFFFF";
    const panelBorder = "#D8D4CC";
    return (
      <div style={{ position: "absolute", inset: 0, zIndex: 30, display: "flex",
        alignItems: topCenter ? "flex-start" : "center",
        justifyContent: isMobile ? "center" : (topCenter ? "center" : "flex-start"),
        paddingLeft: isMobile || topCenter ? 0 : "9%",
        paddingTop: topCenter ? "7%" : 0,
        pointerEvents: "none" }}>
        <motion.div
          ...
          style={{ width, maxHeight: "82vh", overflowY: "auto", textAlign: align, pointerEvents: "auto",
            boxSizing: "border-box",
            WebkitMaskImage: panel ? "none" : "linear-gradient(to bottom, transparent 0, black 24px, black 100%)",
            maskImage: panel ? "none" : "linear-gradient(to bottom, transparent 0, black 24px, black 100%)" }}>
          <TiltBox style={{
            boxSizing: "border-box",
            padding: panel ? "28px 30px" : 0,
            paddingTop: panel ? "28px" : "10px",
            borderRadius: panel ? "18px" : 0,
            background: panel ? panelBg : "none",
            border: panel ? `1px solid ${panelBorder}` : "none",
            boxShadow: panel ? "0 18px 40px -20px rgba(0,0,0,.55)" : "none" }}>
          {children}
          {footer && (
            <div style={{ position: panel ? "static" : "sticky", bottom: 0, paddingTop: "20px", paddingBottom: panel ? 0 : "16px" }}>
              {footer}
            </div>
          )}
          </TiltBox>
  ...
  const progressDots = () => {
    ...
    return (
      <div style={{ marginBottom: "22px" }}>
        ...
        <span style={{ fontSize: "10px", letterSpacing: ".08em", textTransform: "uppercase", color: "#000" }}>
  ...
  const backBtn = (to) => (
    <motion.button onClick={() => setStage(to)}
      ...
      style={{ background: "none", border: "none", color: "#000", fontSize: "10px", letterSpacing: ".08em", textTransform: "uppercase", display: "flex", alignItems: "center", gap: "5px", padding: 0, marginBottom: "18px" }}>
      <IconArrowLeft /> Back
    </motion.button>
  );
  ...
  const ChoiceRow = ({ onClick, accent, children, sub, bars, preview }) => (
    <motion.button onClick={() => { playSfx("select"); onClick?.(); }} variants={listItemV}
      ...
      style={{ display: "block", width: "100%", textAlign: "left", background: "none", border: "none", padding: "9px 0 9px 16px",
        borderLeft: `1px solid #D8D4CC`, position: "relative" }}>
      <span style={{ position: "absolute", left: "-1px", top: 0, bottom: 0, width: "1px", background: "#000",
        transform: "scaleY(var(--c, 0))", transformOrigin: "top", transition: "transform .25s ease" }} />
      <span style={{ display: "flex", alignItems: "baseline", gap: "8px" }}>
        <span style={{ color: "#000", fontSize: "15px", display: "block" }}>{children}</span>
        {bars && (
          <span style={{ display: "inline-flex", gap: "3px", marginLeft: "2px" }}>
            {[1, 2, 3].map(i => (
              <span key={i} style={{ width: "4px", height: "10px", borderRadius: "1px",
                background: i <= bars ? "#000" : "#D8D4CC" }} />
            ))}
          </span>
        )}
      </span>
      {sub && <span style={{ color: "#000", fontSize: "12px", display: "block", marginTop: "2px" }}>{sub}</span>}
      {preview && (
        <span style={{ display: "block", fontFamily: "'Playfair Display',serif", fontStyle: "italic",
          fontSize: "12.5px", color: "#000", opacity: "var(--c, 0)", overflow: "hidden",
  ...
  function StageOverlay() {
    ...
    if (stage === "intro") return (
      <Overlay width="min(560px, 88vw)" align="center" placement="top-center" footer={
        <motion.button onClick={() => setStage("lang")}
          ...
          style={{ display: "inline-flex", alignItems: "center", gap: "10px", background: "#000",
            border: "none", borderRadius: "999px", color: "#FFF", fontSize: "14px", fontWeight: 600,
            letterSpacing: ".01em", padding: "13px 24px 13px 26px", cursor: "pointer", position: "relative", overflow: "hidden",
            boxShadow: "0 3px 10px -4px rgba(0,0,0,.4)" }}>
          <motion.span variants={{ rest: { opacity: 0, scale: 0 }, hover: { opacity: 1, scale: 1 } }}
            transition={{ duration: 0.3, ease: [0.22, 1, 0.36, 1] }}
            style={{ position: "absolute", inset: 0, background: "rgba(255,255,255,.18)" }} />
          ...
          <motion.span variants={{ rest: { x: 0 }, hover: { x: 3 } }} transition={{ duration: 0.25, ease: [0.22, 1, 0.36, 1] }}
            style={{ position: "relative", display: "inline-flex", color: "#FFF" }}>
            <IconArrowRight />
          </motion.span>
        </motion.button>
      }>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.15 }}
          style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "10px", marginBottom: "10px" }}>
          <span style={{ width: "22px", height: "1px", background: "#000" }} />
          <span style={{ fontSize: "10.5px", letterSpacing: ".14em", textTransform: "uppercase", color: "#000", fontWeight: 700 }}>
        ...
        <motion.h1
          ...
          style={{ fontFamily: "'Playfair Display',serif", fontStyle: "italic", fontWeight: 700, fontSize: "clamp(38px,9vw,84px)", color: "#000", marginBottom: "4px", letterSpacing: "-0.025em", lineHeight: 1.05,
        ...
        <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.9, delay: 0.5, ease: [0.22, 1, 0.36, 1] }}
          style={{ width: "120px", height: "1px", background: "linear-gradient(90deg, transparent, #000, transparent)", margin: "0 auto 18px" }} />
        <motion.p initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 0.4, ease: [0.22, 1, 0.36, 1] }}
          style={{ color: "#000", fontWeight: 700, fontSize: "14px", lineHeight: 1.9, maxWidth: "38ch", margin: "0 auto", textShadow: "0 2px 10px rgba(255,255,255,.2)" }}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.6 }}
          style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: "16px", marginTop: "26px" }}>
          {[
            ...
            <button key={key} className="footer-link" onClick={() => setInfoPanel(key)}
              style={{ background: "none", border: "none", color: "#000", fontWeight: 700, fontSize: "11px", letterSpacing: ".05em",
          ...
          <button className="footer-link" onClick={() => setSvgGallery(true)}
            style={{ background: "none", border: "none", color: "#000", fontSize: "11px", letterSpacing: ".05em",
              textTransform: "uppercase", padding: 0, cursor: "pointer", opacity: 0.55 }}
    ...
    if (stage === "lang") return (
      <Overlay panel>
        {backBtn("intro")}
        {progressDots()}
        <h2 style={{ fontFamily: "'Playfair Display',serif", fontStyle: "italic", fontSize: "24px", color: "#000", marginBottom: "20px" }}>Language</h2>
    ...
    if (stage === "mode") return (
      <Overlay panel>
        {backBtn("lang")}
        {progressDots()}
        <h2 style={{ fontFamily: "'Playfair Display',serif", fontStyle: "italic", fontSize: "24px", color: "#000", marginBottom: "20px" }}>Mode</h2>
        <motion.div initial="hidden" animate="visible" variants={listContainer}>
    ...
    if (stage === "disclaimer") {
      ...
      return (
        <Overlay panel footer={
          ...
          {backBtn("mode")}
          {progressDots()}
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontStyle: "italic", fontSize: "22px", color: "#000", marginBottom: "16px" }}>{c.title}</h2>
          <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "22px" }}>
            {c.body.map((p, i) => <p key={i} style={{ fontSize: "13px", lineHeight: 1.7, color: "#000" }}>{p}</p>)}
          </div>
        </Overlay>
      );
    }
    ...
    if (stage === "intensity") return (
      <Overlay panel>
        {backBtn("disclaimer")}
        {progressDots()}
        <h2 style={{ fontFamily: "'Playfair Display',serif", fontStyle: "italic", fontSize: "22px", color: "#000", marginBottom: "20px" }}>{lang === "id" ? "Intensitas" : "Intensity"}</h2>
        <motion.div initial="hidden" animate="visible" variants={listContainer}>
    ...
    if (stage === "scenario") {
      ...
      return (
        <Overlay panel width="min(560px, 90vw)">
          {backBtn("intensity")}
          {progressDots()}
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontStyle: "italic", fontSize: "22px", color: "#000", marginBottom: "14px" }}>{lang === "id" ? "Skenario" : "Scenario"}</h2>
          <motion.button onClick={() => { setScenario(null); setSceneGroupIdx(null); startSession(pendingMode, null, null); }}
            whileHover={{ x: 3 }} whileTap={{ scale: 0.97 }}
            transition={{ duration: 0.18, ease: [0.22, 1, 0.36, 1] }}
              ...
              <div key={gi}>
                ...
                {g.items.map((item, ii) => (
                  <ChoiceRow key={ii} preview={SCENARIO_TEASERS[pendingMode || "formal"]?.[lang || "en"]?.[gi]?.[ii]}
                    onClick={() => { setScenario(item); setSceneGroupIdx(gi); startSession(pendingMode, item, gi); }}>{item}</ChoiceRow>
                ))}
              </div>
            ))}
          </motion.div>
    ...
    if (stage === "summary") return (
      <Overlay panel footer={
        ...
        <h2 style={{ fontFamily: "'Playfair Display',serif", fontStyle: "italic", fontSize: "26px", color: "#000", marginBottom: "16px" }}>{lang === "id" ? "Ringkasan" : "Summary"}</h2>
        <p style={{ fontSize: "13px", lineHeight: 1.8, color: "#000", whiteSpace: "pre-wrap" }}>{summary}</p>
      </Overlay>
    );
  ...
  // All in-session panels (header bar, title chip, transcript) now share a
  // single flat black, no more social/formal split.
  const pillBg = "#FFFFFF";
  const pillBorder = "#D8D4CC";
  ...
  return (
    <div onPointerMove={handlePointerMoveParallax} className="app-shell"
      ...
      <motion.div key={scene.id} style={{ position: "absolute", inset: 0, pointerEvents: "none", zIndex: 0 }}
        ...
        <div style={{ position: "absolute", inset: 0, background: stage === "intro"
          ? `linear-gradient(${isMobile ? 180 : 100}deg, #3A2E22 0%, #241C14 55%, #15110C 100%)`
          : `linear-gradient(${isMobile ? 180 : 165}deg, ${scene.top} 0%, ${scene.bottom} 60%, #0A0807 100%)` }} />
          ...
          <motion.div key={stage === "intro" ? "intro" : scene.id}
            ...
            <motion.div
              ...
              <motion.img
                ...
                animate={stage === "intro"
                  ? { opacity: isMobile ? 0.4 : 1, scale: 1, filter: "blur(0px)" }
                  : reduceMotion
                ...
                style={{ width: "100%", height: "100%", objectFit: "cover",
                  objectPosition: isMobile ? "75% 28%" : "center 28%",
            ...
            <motion.div
              ...
              style={{ position: "absolute", inset: 0, mixBlendMode: "normal", background: "transparent" }} />
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      <div style={{ position: "absolute", top: "max(20px, env(safe-area-inset-top))", right: "20px", zIndex: 40, display: "flex", alignItems: "center",
        ...
        {!ambientPrimed && (
          <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.5 }}
            style={{ fontSize: isMobile ? "10px" : "11px", color: "#000", fontWeight: 700, letterSpacing: ".02em", textAlign: "right", lineHeight: 1.3 }}>
      ...
      <AnimatePresence>
        {inConversation && (
          <motion.div
            ...
            {/* Same pointer-tilt wrapper as the onboarding panel, but with no
                background/border passed in — only the rotateX/Y transform
                applies, so the character tilts in 3D without ever gaining a
                panel behind it. */}
            <TiltBox style={{}}>
            ...
            <div style={{ position: "absolute", top: "100%", marginTop: isMobile ? "-6px" : "2px", left: "50%", transform: "translateX(-50%)", textAlign: "center", whiteSpace: "nowrap" }}>
              <TiltBox style={{ boxSizing: "border-box", padding: "5px 13px", borderRadius: "10px",
                background: pillBg,
                border: `1px solid ${pillBorder}`, boxShadow: "0 8px 18px -12px rgba(0,0,0,.55)" }}>
                <p style={{ fontFamily: "'Playfair Display',serif", fontStyle: "italic", fontSize: "14px", color: "#000" }}>{charMeta.name || charMeta.title}</p>
                <p style={{ fontSize: "9px", color: "#000", marginTop: isMobile ? "0px" : "-3px" }}>{charMeta.title}</p>
                {isLeaning && <p style={{ fontSize: "10px", letterSpacing: ".08em", color: "#000", marginTop: "6px", textTransform: "uppercase" }}>{lang === "id" ? "mendekat..." : "leaning in..."}</p>}
              </TiltBox>
            </div>
          </motion.div>
        )}
      ...
      {inConversation && (
        <>
          {/* Header is now one full-width black bar covering every control,
              flush to the top, ending well above the chat panel below it —
              not individual pills per button anymore. */}
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}
            style={{ position: "absolute", top: 0, left: 0, right: 0, padding: "max(20px, env(safe-area-inset-top)) 28px 16px", display: "flex", alignItems: "center", justifyContent: "space-between",
              background: pillBg, borderBottom: `1px solid ${pillBorder}`, zIndex: 10 }}>
            <motion.button onClick={() => setStage("intro")} whileHover={{ opacity: 0.7 }} whileTap={{ scale: 0.96 }}
              style={{ background: "none", border: "none", color: "#000", fontWeight: 700, fontSize: "13px", fontFamily: "'Inter',sans-serif" }}>Profess</motion.button>
            <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
              <motion.button onClick={() => setTranscriptOpen(o => !o)}
                whileHover={{ opacity: 0.7 }} whileTap={{ scale: 0.96 }}
                transition={{ duration: 0.18 }}
                style={{ background: "none", border: "1px solid #D8D4CC", borderRadius: "8px", color: "#000", fontWeight: 700, fontSize: "11px", padding: "6px 10px" }}>
              ...
              <motion.button onClick={() => setSpeechEnabled(s => !s)} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                style={{ background: "none", border: "none", color: "#000" }}>{speechEnabled ? <IconVolume/> : <IconMute/>}</motion.button>
              <motion.button onClick={resetSession} whileHover={{ scale: 1.1, rotate: -25 }} whileTap={{ scale: 0.9 }}
                style={{ background: "none", border: "none", color: "#000" }}><IconRefresh/></motion.button>
            </div>
          </motion.div>
          ...
          <AnimatePresence>
            {transcriptOpen && (
              <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }}
                transition={{ duration: 0.3, ease: [0.22,1,0.36,1] }}
                style={{ position: "absolute", left: isMobile ? "20px" : "56px", top: isMobile ? "calc(4% + 28dvh + 140px)" : "72px", bottom: isMobile ? "150px" : "150px",
                  width: isMobile ? "88vw" : "min(42%, 520px)", maxWidth: isMobile ? "88vw" : "60%", right: isMobile ? "auto" : "34%",
                  zIndex: 8 }}>
                <TiltBox style={{ height: "100%", overflowY: "auto", padding: "20px 24px", boxSizing: "border-box", borderRadius: "18px",
                  background: pillBg,
                  border: `1px solid ${pillBorder}`,
                  boxShadow: "0 18px 40px -20px rgba(0,0,0,.55)" }}>
                {msgList()}
                {loading && <p style={{ fontSize: "12px", color: "#000" }}>{lang === "id" ? "..." : "thinking..."}</p>}
          ...
          <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, padding: "16px 28px 22px", zIndex: 10, background: "linear-gradient(to top, rgba(7,8,9,0.92), transparent)" }}>
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "stretch" }}>
              <textarea ref={textareaRef} value={input} onChange={handleTA} onKeyDown={handleKeyDown}
                ...
                className="input-glow" style={{ "--ac": sessionAccent, flex: 1, minHeight: "48px", maxHeight: "160px", resize: "none", background: "#FFFFFF",
                  border: isListening ? "1px solid #BC5A5A" : "1px solid #D8D4CC", borderRadius: "12px", color: "#000",
                  padding: "13px 14px", fontFamily: "inherit", fontSize: "14px" }} />
              {isSpeaking && <motion.button onClick={stopSpeech} whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.92 }} style={{ background: "#FFFFFF", border: "1px solid #D8D4CC", borderRadius: "10px", padding: "12px", color: "#000" }}><IconStop/></motion.button>}
              {/* One unified panel — top row is End session (split with Try again
                  when it's available), bottom row splits mic | send. Height is
                  pinned to the textarea's own min-height so it reads as one
                  control surface, not three separate floating buttons. */}
              <div style={{ display: "flex", flexDirection: "column", width: "104px", height: "48px",
                borderRadius: "10px", overflow: "hidden", border: `1px solid ${pillBorder}`, background: pillBg }}>
                <div style={{ display: "flex", flex: 1, borderBottom: `1px solid ${pillBorder}` }}>
                  {lastExchange && (
                    <motion.button onClick={tryAgain} whileHover={{ opacity: 0.7 }} whileTap={{ scale: 0.95 }}
                      style={{ flex: 1, background: "none", border: "none", borderRight: `1px solid ${pillBorder}`,
                        color: "#000", fontWeight: 700, fontSize: "9.5px" }}>
                      {lang === "id" ? "Coba lagi" : "Try again"}
                    </motion.button>
                  )}
                  <motion.button onClick={endSession} whileHover={{ opacity: 0.7 }} whileTap={{ scale: 0.95 }}
                    style={{ flex: 1, background: "none", border: "none", color: "#000", fontWeight: 700, fontSize: "9.5px" }}>
                    {lang === "id" ? "Akhiri" : "End session"}
                  </motion.button>
                </div>
                <div style={{ display: "flex", flex: 1 }}>
                  <motion.button onClick={toggleMic} className={isListening ? "mic-active" : ""} whileHover={{ opacity: 0.7 }} whileTap={{ scale: 0.95 }}
                    style={{ flex: 1, background: "none", border: "none", borderRight: `1px solid ${pillBorder}`,
                      color: "#000", display: "flex", alignItems: "center", justifyContent: "center" }}><IconMic/></motion.button>
                  <motion.button id="psend" onClick={sendMessage} disabled={loading || !input.trim()} whileHover={!loading && input.trim() ? { opacity: 0.7 } : {}} whileTap={!loading && input.trim() ? { scale: 0.95 } : {}}
                    style={{ flex: 1, background: "none", border: "none", color: "#000",
                      display: "flex", alignItems: "center", justifyContent: "center", opacity: loading || !input.trim() ? .4 : 1 }}><IconSend/></motion.button>
                </div>
              </div>
            </div>
          </div>

...
export default function Profess() {
  ...
  function StageOverlay() {
    ...
    if (stage === "intro" && !videoIntroDone) return null;
    ...
    <div onPointerMove={handlePointerMoveParallax} className="app-shell"
      ...
      <div style={{ position: "absolute", inset: 0, zIndex: 0, overflow: "hidden", pointerEvents: "none" }}>
        <AnimatePresence mode="sync">
          {/* On desktop, hold the mountain backdrop hidden until the video's
              own exit fade has actually finished — otherwise this entrance
              plays out fully while still hidden behind the opaque video and
              is never seen. Mobile has no video, so it's already "done". */}
          {(stage !== "intro" || videoIntroDone) && (
          <motion.div key={stage === "intro" ? "intro" : scene.id}
            ...
            <motion.div style={{ position: "absolute", inset: "-8%", x: photoX, y: photoY }}>
              <motion.img
                ...
                initial={stage === "intro" ? { opacity: 0, scale: 1.14, filter: "blur(10px)" } : { opacity: 0, scale: 1.05 }}

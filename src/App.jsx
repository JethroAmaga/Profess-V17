...
export default function Profess() {
  ...
  const galleryChars = useMemo(() => {
    ...
    const buildRow = (ch, key, title) => ({
      key, title,
      moods: GALLERY_MOODS.map((mood) => ({ mood, svg: buildSVG(ch, mood, false, "portrait") })),
    });
  ...
  useEffect(() => {
    ...
    // suspended forever. touchstart is added since mobile never fires
    // pointermove (no mouse), which previously left ambient sound desktop-only.
    const arm = () => {

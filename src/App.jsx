...
// ─── Parallax hero images — adapted from Aceternity's ParallaxHeroImages ───
// (no Tailwind/cn in this project, so positions/styling are plain inline
// objects instead of utility classes). A scattered photo collage that drifts
// opposite the cursor, used as a decorative teaser on the mode-selection screen.
const PARALLAX_POSITIONS = {
  "top-left": { top: "10%", left: "5%" },
  "top-right": { top: "10%", right: "5%" },
  "mid-left": { top: "40%", left: "3%" },
  "mid-right": { top: "40%", right: "3%" },
  "bottom-left": { top: "70%", left: "6%" },
  "bottom-right": { top: "70%", right: "6%" },
  "far-left": { top: "54%", left: "1%" },
  "far-right": { top: "54%", right: "1%" },
};
const PARALLAX_ORDER = ["top-left", "top-right", "mid-left", "mid-right", "bottom-left", "bottom-right", "far-left", "far-right"];
const PARALLAX_DEPTHS = [0.3, 0.35, 0.9, 0.85, 0.4, 0.45, 0.25, 0.2];

function ParallaxHeroImages({ images, isMobile }) {
  ...
  const items = images.slice(0, 8).map((src, i) => ({
    src, position: PARALLAX_ORDER[i], depth: PARALLAX_DEPTHS[i], delay: i * 0.12,
  }));

  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden", pointerEvents: "none" }}>
      {items.map((item, i) => (
        <ParallaxHeroImage key={`${item.src}-${i}`} {...item} smoothMouseX={smoothMouseX} smoothMouseY={smoothMouseY} isMobile={isMobile} />
      ))}
    </div>
  );
}

function ParallaxHeroImage({ src, position, depth, delay, smoothMouseX, smoothMouseY, isMobile }) {
  const maxOffset = 36;
  ...
  const pos = PARALLAX_POSITIONS[position];

  return (
    <motion.div
      style={{ position: "absolute", top: pos.top, left: pos.left, right: pos.right,
        x: isMobile ? 0 : translateX, y: isMobile ? 0 : translateY, zIndex: Math.round(depth * 10) }}
      initial={{ opacity: 0, filter: "blur(20px)", scale: 0.9 }}
      animate={{ opacity: 1, filter: "blur(0px)", scale: 1 }}
      transition={{ duration: 0.8, delay, ease: [0.25, 0.1, 0.25, 1] }}>
      <img src={src} alt="" loading="lazy" decoding="async"
        style={{ width: isMobile ? "84px" : "168px", height: isMobile ? "63px" : "126px", objectFit: "cover",
          borderRadius: "10px", boxShadow: "0 6px 24px -8px rgba(0,0,0,.6)", border: "1px solid rgba(255,255,255,.08)",
...
export default function Profess() {
  ...
  return (
    <div onPointerMove={handlePointerMoveParallax} className="app-shell"
      ...
      {/* Mode-selection teaser — a scattered collage of one photo per scenario,
          drifting opposite the cursor. A preview of the worlds on offer before
          the user commits to Formal or Social. */}
      {stage === "mode" && (
        <div style={{ position: "absolute", inset: 0, zIndex: 5 }}>
          <ParallaxHeroImages isMobile={isMobile} images={[
            ...SCENES.formal.map(s => `/video/${s.backdrop[0]}`),
            ...SCENES.social.map(s => `/video/${s.backdrop[0]}`),
          ]} />
        </div>
      )}

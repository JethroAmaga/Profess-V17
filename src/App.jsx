...
const PLAYLISTS = [
  { id: "37i9dQZF1DWXjo7kaCGuCU", label: "Ambient Focus", desc: "For formal sessions" },
  { id: "37i9dQZF1DWYoYGBbGKurt", label: "Lo-Fi Chill",   desc: "For social sessions" },
  { id: "37i9dQZF1DX4sWSpwq3LiO", label: "Deep Focus",    desc: "For intense sessions" },
];
    ...
    return (
      <div className="screen-enter" style={{ ...BASE, overflowY:"auto", filter:warmMode?"sepia(0.18) saturate(1.2) brightness(1.02)":"none", transition:"filter 1.2s ease" }}>
        ...
        <div style={{ position:"relative", height:"100vh", display:"flex", flexDirection:"column", overflow:"hidden", paddingBottom:"0" }}>
          ...
          <div style={{ position:"absolute", right:0, top:0, height:"100%", width:"45%", opacity: warmMode ? 0.55 : 0.35, transition:"opacity 1.2s ease", pointerEvents:"none", zIndex:1 }}>
            <svg viewBox="0 0 100 100" preserveAspectRatio="xMidYMid slice" style={{ width:"100%", height:"100%" }}>
              ...
              {/* Left sofa — backrest */}
              <rect x="5" y="52" width="36" height="22" rx="3" fill="#221408" stroke="#2A1A0A" strokeWidth="0.25"/>
              {/* Left sofa — body */}
              <rect x="5" y="60" width="36" height="15" rx="2" fill="#1A120A" stroke="#2A1A0A" strokeWidth="0.25"/>
              {/* Left sofa — armrests */}
              <rect x="4" y="60" width="4.5" height="12" rx="2" fill="#221408"/>
              <rect x="37.5" y="60" width="4.5" height="12" rx="2" fill="#221408"/>
              {/* Left sofa — cushion hint */}
              <rect x="9" y="60.5" width="16" height="14" rx="1.5" fill="#1E1510" opacity="0.6"/>
              <rect x="27" y="60.5" width="13" height="14" rx="1.5" fill="#1E1510" opacity="0.6"/>
              {/* Left character */}
              ...
              {/* Right sofa — backrest */}
              <rect x="59" y="54" width="30" height="20" rx="3" fill="#181008" stroke="#221208" strokeWidth="0.25"/>
              {/* Right sofa — body */}
              <rect x="59" y="61" width="30" height="14" rx="2" fill="#181008" stroke="#221208" strokeWidth="0.25"/>
              {/* Right sofa — armrests */}
              <rect x="57.5" y="61" width="4" height="11" rx="2" fill="#1E1208"/>
              <rect x="86" y="61" width="4" height="11" rx="2" fill="#1E1208"/>
              {/* Right sofa — cushion hint */}
              <rect x="63" y="61.5" width="13" height="13" rx="1.5" fill="#1C1008" opacity="0.6"/>
              <rect x="78" y="61.5" width="10" height="13" rx="1.5" fill="#1C1008" opacity="0.6"/>
              {/* Right character */}
              ...
              {/* Radio side table */}
              <rect x="0.5" y="70" width="14" height="2" rx="0.5" fill="#2A1808" opacity="0.9"/>
              <rect x="1.5" y="72" width="3" height="3.5" rx="0.3" fill="#221408"/>
              <rect x="9.5" y="72" width="3" height="3.5" rx="0.3" fill="#221408"/>
              {/* Radio body */}
              <rect x="1" y="62" width="13" height="8" rx="1" fill="#4A4A52" stroke="#5A5A62" strokeWidth="0.4"/>
              {/* Radio speaker grille */}
              <line x1="2.5" y1="63.5" x2="9" y2="63.5" stroke="#3A3A42" strokeWidth="0.45" opacity="0.9"/>
              <line x1="2.5" y1="65" x2="9" y2="65" stroke="#3A3A42" strokeWidth="0.45" opacity="0.9"/>
              <line x1="2.5" y1="66.5" x2="9" y2="66.5" stroke="#3A3A42" strokeWidth="0.45" opacity="0.9"/>
              {/* Radio knobs */}
              <circle cx="11" cy="64.2" r="1" fill="#6A6A72"/>
              <circle cx="11" cy="67" r="1" fill="#6A6A72"/>
              {/* Radio antenna */}
              <line x1="13.2" y1="62" x2="16.5" y2="56.5" stroke="#5A5A62" strokeWidth="0.6" style={showPlayer ? {animation:"antennaP 1s ease-in-out infinite"} : {opacity:0.5}}/>
          ...
          {/* Radio hint text */}
          <div style={{ position:"absolute", left:"57%", top:"64%", pointerEvents:"none", zIndex:6, textAlign:"center", transition:"color 0.8s ease" }}>
          ...
          {/* Radio click overlay */}
          <div onClick={() => { hasOpenedMusic.current = true; setShowMusicSuggest(false); setShowPlayer(p => !p); }} title="Toggle music player" style={{ position:"absolute", left:"55%", top:"62%", width:"7%", height:"10%", cursor:"pointer", zIndex:7 }}/>
    ...
    <div style={{ ...BASE, display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ padding:`0 ${isMobile?"16px":"32px"}`, height:isMobile?"48px":"56px", borderBottom:"1px solid #141414", display:"flex", alignItems:"center", gap:"16px", flexShrink:0 }}>
        ...
        <div style={{ marginLeft:"auto", display:"flex", gap:"8px", alignItems:"center" }}>
          {hasOpenedMusic.current && (
            <button onClick={() => setShowPlayer(p => !p)} style={{ background:"none", border:"none", padding:"6px 4px", display:"flex", alignItems:"flex-end", gap:"1.5px", cursor:"pointer" }} title="Music">
              {[0,.1,.2,.1,0].map((d,i) => <div key={i} style={{ width:"2px", height:`${4+i*2.5}px`, background:charMeta.accent, borderRadius:"1px", animation:`waveBar .6s ease-in-out ${d}s infinite`, opacity:0.5 }}/>)}
            </button>
          )}
...
function MusicWidget({ activePlaylist, setActivePlaylist, onClose }) {
  return (
    <div style={{ position:"fixed", bottom:"24px", left:"24px", right:"auto", zIndex:500, background:"#0A0A0A", border:"1px solid #2A2520", width:"300px", boxShadow:"0 8px 32px rgba(0,0,0,0.6)" }}>
      {/* Header */}
      ...
      <div style={{ padding:"8px 16px", display:"flex", gap:"6px" }}>
        {PLAYLISTS.map((pl, i) => (
          <button key={i} onClick={() => setActivePlaylist(i)}
            style={{ padding:"4px 10px", border:`1px solid ${activePlaylist===i?"#C8A870":"#1E1E1E"}`, color:activePlaylist===i?"#C8A870":"#4A4540", fontFamily:"'Inter',sans-serif", fontWeight:300, fontSize:"10px", letterSpacing:".06em", background:"none", cursor:"pointer", transition:"border-color .2s, color .2s" }}>
            {pl.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding:"8px 16px" }}>
        <span style={{ fontFamily:"'Inter',sans-serif", fontWeight:300, fontSize:"10px", color:"#3A3530", letterSpacing:".04em" }}>{PLAYLISTS[activePlaylist].desc}</span>
      </div>
    </div>
  );
}

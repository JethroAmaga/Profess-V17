...
// layer behind the character. Not literal scenario photography — atmosphere,
// not illustration. Each scene carries a small pool of seeds (`scene.photos`)
// instead of one fixed seed, so repeat sessions in the same scenario group
// don't always land on the identical backdrop — picked once per scene via
// pickPhotoSeed.
...
// offline, picsum outage), the <img onError> handler swaps to this instead
...
const SFX = {
  ...
  select:  (ctx, t) => tone(ctx, { freq: 660, start: t, dur: 0.09, type: "triangle", gain: 0.06 }),
...
export default function Profess() {
  ...
  // Reset the fallback flag whenever the seed changes so a new scene always
  // retries the real photo first, rather than staying stuck on art forever
  // after one failed load.
  const [photoFailed, setPhotoFailed] = useState(false);
  useEffect(() => { setPhotoFailed(false); }, [photoSeed]);
        ...
        <AnimatePresence mode="sync">
          <motion.div key={scene.id}
            ...
            <motion.div style={{ position: "absolute", inset: "-4%", x: photoX, y: photoY }}>
              ...
              <motion.img
                src={photoFailed ? sceneArtFor(photoSeed, 1200, 1600, scene) : photoFor(photoSeed, 1200, 1600)}

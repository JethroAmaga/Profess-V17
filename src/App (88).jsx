...
//
// Keyword is required, not optional: LoremFlickr's keyword-less endpoint
// pulls from a generic demo gallery that's heavy on cat photos, which read
// as random/off-brand here. Each scene already carries thematic keywords in
// SCENES[...].photos (e.g. "library-stacks", "office-dusk") — use the first
// one so the photo actually matches the scene instead of being arbitrary.
const photoFor = (scene, w = 1000, h = 1400) => {
  const seed = scene?.id || "default";
  const keyword = (scene?.photos?.[0] || seed).replace(/-/g, ",");
  let lock = 0;
  for (let i = 0; i < seed.length; i++) lock = (lock * 31 + seed.charCodeAt(i)) >>> 0;
  return `https://loremflickr.com/${w}/${h}/${encodeURIComponent(keyword)}?lock=${lock % 100000}`;
};
...
// Served from Pexels CDN (free, no attribution required, hotlink allowed).
// Multiple sources for resilience — if one CDN fails or is CORS-blocked,
// the next one is tried via the <source> fallback chain in IntroVideoScreen.
const VIDEO_SOURCES = [
  "https://videos.pexels.com/video-files/4763031/4763031-hd_1920_1080_25fps.mp4",
  "https://videos.pexels.com/video-files/4763031/4763031-sd_640_360_25fps.mp4",
];

// ─── Backdrop art fallback — generated locally, no network involved. If the
// real photo above fails to load (host blocked by the viewer's network,
// offline, source outage), the <img onError> handler swaps to this instead
// of leaving the backdrop blank. Abstract seeded color-light using the
// scene's own palette, so even the fallback still reads as "this scene."
...
const sceneArtFor = (seed, w = 1000, h = 1400, scene = DEFAULT_SCENE) => {
  const rand = seededRand(String(seed));
  const top = scene?.top || "#2A2018";
  const bottom = scene?.bottom || "#0E0B08";
  const glow = scene?.floor || "#C9985E";
  const blobs = Array.from({ length: 4 }, (_, i) => {
    const cx = Math.round(rand() * w);
    const cy = Math.round(rand() * h);
    const r = Math.round(w * (0.35 + rand() * 0.35));
    const color = i % 2 === 0 ? glow : top;
    const op = (0.16 + rand() * 0.16).toFixed(2);
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}">
    <defs>
      ...
      <filter id="b"><feGaussianBlur stdDeviation="${Math.round(w * 0.06)}" /></filter>
    </defs>
...
function IntroVideoScreen({ onDone, lang, reduceMotion }) {
...
export default function Profess() {
  ...
  const [stage, setStage] = useState("video-intro"); // video-intro|intro|lang|mode|disclaimer|intensity|scenario|conversation|summary
  ...
  useEffect(() => { photoLoadedRef.current = photoLoaded; }, [photoLoaded]);
  useEffect(() => {
    setPhotoFailed(false);
    setPhotoLoaded(false);
    photoLoadedRef.current = false;
    // LoremFlickr resolves its keyword search live, which can take several
    // seconds on a cold request. Rather than leave the scene blank/waiting,
    // give the real photo a window to arrive and fall back to the
    // illustrated art if it hasn't — the art renders instantly underneath
    // either way (see the layered <img> pair below), so there's never a
    // visibly empty backdrop.
    const t = setTimeout(() => { if (!photoLoadedRef.current) setPhotoFailed(true); }, 2500);
    return () => clearTimeout(t);
  }, [scene.id]);
  ...
  return (
    <div onPointerMove={handlePointerMoveParallax} className="app-shell"
      ...
      <AnimatePresence>
        {stage === "video-intro" && (
          <IntroVideoScreen key="video-intro" lang={lang} reduceMotion={reduceMotion}
        ...
        <AnimatePresence mode="sync">
          <motion.div key={scene.id}
            ...
            <motion.div style={{ position: "absolute", inset: "-4%", x: photoX, y: photoY }}>
              {/* Illustrated art renders immediately underneath — it's already a
                  local asset, no network round-trip — so the scene is never blank
                  while the real photo (LoremFlickr resolves its keyword search
                  live, which can take seconds) is still loading. The photo crossfades
                  on top once decoded; if it stalls past the timeout in the effect
                  above, photoFailed flips true and this base layer is simply left
                  showing instead of swapping the <img> src underneath it. */}
              {photoFailed && (
                <img src={sceneArtFor(scene.id, 1200, 1600, scene)} alt="" loading="eager"
                  style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover",
                    objectPosition: "center 28%", filter: "grayscale(1) contrast(1.15) brightness(.78)" }} />
              )}
              {/* Opacity used to be gated behind `scenePhotoLoaded` (set via onLoad),
                  so the image animated in twice: once on mount (target 0, since the
                  flag wasn't set yet) and a second time once the network fetch
                  finished and the flag flipped true — read as a double entrance flash.
                  Animating straight to the target on mount removes the second pass;
                  the only visible "delay" left is the image's own decode time. */}
              {!photoFailed && (
              <motion.img
                ...
                animate={reduceMotion
                  ...
                  : { opacity: isMobile ? 0.4 : 1, scale: [1.0, 1.12, 1.05, 1.12] }}
            ...
            <div style={{ position: "absolute", inset: 0, mixBlendMode: "multiply",
              background: `linear-gradient(200deg, ${scene.top}00 0%, ${scene.bottom}cc 75%, #07050480 100%)` }} />

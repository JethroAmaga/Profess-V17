...
// Pants — deliberately darker/cooler than the shirt families above so the
// torso and legs read as two separate garments rather than one flat suit.
const PANTS_FAMILIES = {
  formal: ["#3A2A12","#2E2210","#42300F","#352714","#281C0C","#3E2C16"],
  social: ["#3A1416","#2C1012","#401A1A","#301214","#240C0E"],
};
...
const generateChar = (roleKey, forcedGender = null, mode = "formal") => {
  ...
  const trim = TRIM_FAMILIES[socialRoles.includes(roleKey) ? "social" : (mode === "social" ? "social" : "formal")];
  const pantsFamily = PANTS_FAMILIES[socialRoles.includes(roleKey) ? "social" : (mode === "social" ? "social" : "formal")];
  const pantsColor = pantsFamily[hueIdx % pantsFamily.length];
...
const CHARS = {
  default: {
    ...
    bodyColor:"#5C4E3C", pantsColor:"#221E18", trim:"#D9A05C", tie:null, isCoach:true,
  },
};

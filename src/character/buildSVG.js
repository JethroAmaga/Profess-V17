...
`,
  ...
  // Shoe — a rounded vertical cylinder (rim, body, sole) rather than a flat
  // ellipse blob, so the foot reads as a boot/shoe shape instead of a paddle.
  const shoeSVG = (cx) => `
    <rect x="${cx-9}" y="206" width="18" height="20" rx="9" fill="url(#pantsGrad)"/>
    <ellipse cx="${cx}" cy="208" rx="8" ry="3.5" fill="${lighten(p,20)}" opacity=".45"/>
    <path d="M${cx-9} 213 Q${cx} 217 ${cx+9} 213" stroke="${darken(p,22)}" stroke-width="1.2" fill="none" opacity=".5"/>
    <ellipse cx="${cx}" cy="224" rx="10" ry="5" fill="${darken(p,34)}"/>`;
  ...
  if (isCoach) {
    ...
    bodySVG = `
      ...
      <path d="M38 182 Q38 196 52 200 L108 200 Q122 196 122 182 L122 190 Q122 204 108 208 L52 208 Q38 204 38 190 Z" fill="${darken(b,30)}"/>

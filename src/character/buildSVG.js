...
import { CHARS, MOOD_DATA, darken, lighten } from "./characterData.js";
...
`,
  ...
  // per-role branches below bake in furniture (desks, picnic blankets, café
  // tables) and several use a short "seated" torso that only reads correctly
  // with that furniture covering the gap below it — in isolation it looks
  // like a disconnected box floating mid-figure. Portrait mode always uses
  // the same clean full-body silhouette (tapered torso + shared legs/feet)
  // so every role renders as a plain, comparable character card.
  const isPortrait = scene === "portrait";

  if (isCoach || isPortrait) {
...
break;
  ...
  return `<svg viewBox="0 0 160 240" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%;overflow:visible">
    ...
    <g opacity=".5" style="mix-blend-mode:screen">
      <path d="M92 134 Q94 155 93 176" fill="none" stroke="${lighten(b,30)}" stroke-width="5" stroke-linecap="round"/>
      <path d="M110 140 Q112 158 109 176" fill="none" stroke="${lighten(b,30)}" stroke-width="6" stroke-linecap="round"/>
    </g>
    <circle cx="80" cy="82" r="42" fill="url(#faceGrad)"/>
    <ellipse cx="64" cy="68" rx="14" ry="8" fill="#ffffff" opacity=".16"/>
    ${hairSVG}
    <ellipse cx="38" cy="86" rx="7" ry="9" fill="${darken(s,6)}"/>
    <ellipse cx="122" cy="86" rx="7" ry="9" fill="${darken(s,6)}"/>
    ${beardSVG}${glassesSVG}
    ${browsSVG && browAnim

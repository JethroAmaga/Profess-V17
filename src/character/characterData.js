...
// ─── Outfit palette — tied to the UI's session accent (App.jsx ACCENT) so the
// character's clothing never reads as a different design language than the
// chrome around it. Formal sessions: amber/umber/coffee. Social: terracotta/wine.
// Keep these hexes in sync with ACCENT.formal / ACCENT.social in App.jsx.
// Widened on purpose: the previous 5-6 entries per family were near-identical
// dark umber shades, so most roles ended up visually indistinguishable from
// each other and from the coach's own outfit. These span the gold→bronze
// axis (formal) and the wine→rust axis (social) — distinct hues, but every
// one of them stays inside Profess's gold/red brand palette.
const OUTFIT_FAMILIES = {
  formal: ["#8C6A2E","#A6791F","#6B4C18","#9C5A28","#7A4420","#B98532","#5E4012","#8F7536"],
  social: ["#7A2030","#921F2B","#5C1620","#8C3326","#6B1A28","#A33C2E","#4E1420","#9A4438"],
};
const TRIM_FAMILIES = { formal: "#D9A05C", social: "#E08572" };

// ─── Generate character ────────────────────────────────────────────────────
const generateChar = (roleKey, forcedGender = null, mode = "formal") => {
  ...
  // Roles written as peers/young adults — no beard regardless of gender roll,
  // since a full beard reads as visibly older than the role implies.
  const youngRoles = ["friend_female","friend_male","best_friend","ex_partner","sibling",
    "crush","romantic_interest","date","blind_date","colleague","subordinate","mentee",
    "junior","stranger","new_acquaintance","classmate","customer_service",
    "teman_ospek","anggota_tim_debat","senior_organisasi"];
  const beard = gender === "m" && !youngRoles.includes(roleKey) && Math.random() > 0.6;
  ...
  // Body color groups
  const socialRoles = ["friend_female","friend_male","best_friend","ex_partner","crush","romantic_interest","date","blind_date","parent","grandparent","colleague","manager","subordinate","mentor","mentee","senior","junior","stranger","new_acquaintance","neighbor","classmate","alumni","host","guest","fellow_passenger","customer_service","sibling","pak_rt","calon_mertua","senior_organisasi","teman_ospek","anggota_tim_debat"];
  const family = OUTFIT_FAMILIES[socialRoles.includes(roleKey) ? "social" : (mode === "social" ? "social" : "formal")];
  // Stable per-role pick (not random) so a given role keeps its outfit shade across regenerations
  const hueIdx = Math.abs([...roleKey].reduce((a, ch) => a + ch.charCodeAt(0), 0)) % family.length;
  const bodyColor = family[hueIdx];
  // Pants and trim are derived from the shirt's own hue (not a separate
  // palette) so the whole outfit stays in one color family per the shirt —
  // e.g. a gold-brown shirt gets gold-brown pants and shading, never a
  // mismatched maroon or cool-grey leg.
  const pantsColor = darken(bodyColor, 38);
  const trim = lighten(bodyColor, 46);

  const tieRoles = ["interviewer","defense_lawyer","prosecutor","negotiator","ceo","executive","diplomat","board_member","acquirer","investor"];
  const tie = tieRoles.includes(roleKey) && gender === "m" ? darken(trim, 60) : null;

  return { name, gender, ethnicity: eth, roleKey, accent: ACCENTS[eth]||"#C8B89A", bg: BGS[eth]||"#161410", skin, hair, hairLong, hairStyle, glasses, beard, bodyColor, pantsColor, trim, tie, outfitStyle };
};

// Static CHARS for default/coach only; others generated dynamically.
// bodyColor here is the formal-mode default (coklat emas); App.jsx swaps it
// to bodyColorSocial when the session mode is social. pantsColor stays fixed
// across modes — kept near-black so it reads as a clear, contrasting break
// from either shirt color rather than blending into it.
const CHARS = {
  default: {
    name:"Profess", title:"Your Coach", accent:"#C8B89A", bg:"#16130F",
    skin:"#D4A87A", hair:"#E8E4DC", hairLong:false, glasses:true, beard:false,
    bodyColor:"#9C7B3A", bodyColorSocial:"#8C2030", pantsColor:"#14110D", trim:"#D9A05C", tie:null, isCoach:true,
  },
};
...
// Proportional highlight — mixes toward white by a fraction of each channel's
// remaining headroom, instead of adding a flat amount. Flat-amount lighten()
// blows already-light colors (e.g. light blonde hair, #E8D090) out to
// near-white; this keeps the hue visible regardless of how light the base is.
...
export { SKIN, HAIR, ETHNICITIES, pick, ROLE_TITLES, NAME_POOL, generateChar, CHARS, MOOD_DATA, darken, lighten, tint };

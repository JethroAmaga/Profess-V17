...
// ─── Real photography, sourced by deterministic seed (Picsum: real stock
// photographs, no API key, stable per-seed result) — used as a soft "set"
// layer behind the character and as small avatar stills on choice rows.
// Not literal scenario photography — atmosphere, not illustration. Each
// scene now carries a small pool of seeds (`scene.photos`) instead of one
// fixed seed, so repeat sessions in the same scenario group don't always
// land on the identical backdrop — picked once per scene via pickPhotoSeed.
const photoFor = (seed, w = 1000, h = 1400) =>
  `https://picsum.photos/seed/${encodeURIComponent(seed)}/${w}/${h}`;
const pickPhotoSeed = (scene) => {
  const pool = scene?.photos?.length ? scene.photos : [scene?.id || "default"];
...
const SFX = {
  ...
  select:  (ctx, t) => tone(ctx, { freq: 660, start: t, dur: 0.07, type: "triangle", gain: 0.035 }),
...
function useAmbientSound(enabled, accentHex) {
  ...
  useEffect(() => {
    ...
    if (!nodesRef.current) {
      ...
      for (let i = 0; i < bufferSize; i++) {
        ...
        data[i] = last * 4.5; // brown-ish noise, gentle
      }
      ...
      src.start();
      nodesRef.current = { src, filter, gain, ctx };
    }
  ...
  useEffect(() => {
    ...
    nodesRef.current.filter.frequency.linearRampToValueAtTime(
      360 + (hash % 220), nodesRef.current.ctx.currentTime + 1.2
    );
  }, [accentHex]);
...
export default function Profess() {
  ...
  const galleryChars = useMemo(() => {
    ...
    const roleEntries = Object.keys(ROLE_TITLES).map((key) => {
      const ch = generateChar(key);
      return { key, title: ROLE_TITLES[key], svg: buildSVG(ch, "neutral", false, "portrait") };
    });
    return [{ key: "coach", title: CHARS.default.title, svg: buildSVG(CHARS.default, "neutral", false, "portrait") }, ...roleEntries];
  }, [svgGallery]);
  ...
  // Browsers block audio until a real user gesture happens, so true autoplay
  // from page-load is off the table. Instead we arm the very first gesture
  // anywhere on the page (a cursor move counts) to switch ambient sound on,
  // so it starts within the first second or two without the user ever having
  // to find and press the speaker icon themselves.
  useEffect(() => {
    if (ambientPrimed) return;
    const arm = () => { setAmbientOn(true); setAmbientPrimed(true); };
  ...
  const photoSeed = useMemo(() => pickPhotoSeed(scene), [scene.id]);
  useAmbientSound(ambientOn, sessionAccent);
      ...
      <Overlay footer={
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.6 }}
          ...
          {[
            ...
            <button key={key} onClick={() => setInfoPanel(key)}
              style={{ background: "none", border: "none", color: "#9C8E7C", fontSize: "11px", letterSpacing: ".05em",
                textTransform: "uppercase", padding: 0, cursor: "pointer", borderBottom: "1px solid transparent" }}
          ...
          {/* Design-review only: not user-facing copy, so left untranslated and
              visually de-emphasized (dotted underline) to mark it as a tool. */}
          <button onClick={() => setSvgGallery(true)}
            style={{ background: "none", border: "none", color: "#5A5044", fontSize: "11px", letterSpacing: ".05em",
              textTransform: "uppercase", padding: 0, cursor: "pointer", borderBottom: "1px dotted #5A5044" }}
            onMouseEnter={(e) => e.currentTarget.style.color = sessionAccent}
        ...
        <AnimatePresence mode="sync">
          <motion.div key={scene.id}
            ...
            <motion.div style={{ position: "absolute", inset: "-4%", x: photoX, y: photoY }}>
              ...
              <motion.img src={photoFor(photoSeed, 1200, 1600)} alt="" loading="eager"
                initial={{ opacity: 0, scale: 1.0 }}
                animate={{ opacity: isMobile ? 0.4 : 1, scale: 1.12 }}
                transition={{ opacity: { duration: 0.9, ease: [0.22, 1, 0.36, 1] }, scale: { duration: 18, ease: "easeOut" } }}
      ...
      <AnimatePresence>
        {inConversation && (
          <motion.div
            ...
            animate={{ left: isMobile ? "50%" : "94%", top: isMobile ? "4%" : "max(330px, min(50%, calc(100dvh - 490px)))", x: isMobile ? "-50%" : "-100%", y: isMobile ? "0%" : "-50%", opacity: 1, scale: 1 }}
            ...
            <div style={{ position: "absolute", top: "100%", marginTop: isMobile ? "6px" : "18px", left: "50%", transform: "translateX(-50%)", textAlign: "center", whiteSpace: "nowrap" }}>
          ...
          <AnimatePresence>
            {transcriptOpen && (
              <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }}
                transition={{ duration: 0.3, ease: [0.22,1,0.36,1] }}
                style={{ position: "absolute", left: 0, top: isMobile ? "calc(4% + 28dvh + 60px)" : "72px", bottom: isMobile ? "20px" : "150px",
          ...
          <motion.div key="svg-gallery-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div onClick={(e) => e.stopPropagation()}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: "18px" }}>
                {galleryChars.map((g) => (
                  <div key={g.key} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "6px" }}>
                    <div style={{ width: "100px", height: "100px" }} dangerouslySetInnerHTML={{ __html: g.svg }} />
                    <p style={{ fontSize: "10px", color: "#AFA290", textAlign: "center", lineHeight: 1.3 }}>{g.title}</p>
                  </div>
                ))}
              </div>
            </motion.div>

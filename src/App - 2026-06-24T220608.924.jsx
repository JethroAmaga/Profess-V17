        ...
        <>
          ...
          <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, padding: "16px 28px 22px", zIndex: 10, background: "linear-gradient(to top, rgba(7,8,9,0.92), transparent)" }}>
            <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px", marginBottom: "8px" }}>
              {lastExchange && <motion.button onClick={tryAgain} whileHover={{ opacity: 0.85 }} whileTap={{ scale: 0.95 }} style={{ background: pillBg, border: `1px solid ${pillBorder}`, borderRadius: "999px", color: "#000", fontWeight: 700, fontSize: "11px", padding: "6px 12px" }}>{lang === "id" ? "Coba lagi" : "Try again"}</motion.button>}
              <motion.button onClick={endSession} whileHover={{ opacity: 0.85 }} whileTap={{ scale: 0.95 }} style={{ background: pillBg, border: `1px solid ${pillBorder}`, borderRadius: "999px", color: "#000", fontWeight: 700, fontSize: "11px", padding: "6px 12px" }}>{lang === "id" ? "Akhiri" : "End session"}</motion.button>
            </div>
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "flex-end" }}>
              ...
              <motion.button onClick={toggleMic} className={isListening ? "mic-active" : ""} whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.92 }}
                style={{ background: "#FFFFFF", border: isListening ? "1px solid #BC5A5A" : "1px solid #D8D4CC", borderRadius: "10px", padding: "12px", color: "#000" }}><IconMic/></motion.button>
              <motion.button id="psend" onClick={sendMessage} disabled={loading || !input.trim()} whileHover={!loading && input.trim() ? { scale: 1.06 } : {}} whileTap={!loading && input.trim() ? { scale: 0.9 } : {}}
                style={{ background: "#FFFFFF", border: "1px solid #D8D4CC", borderRadius: "10px", padding: "12px 16px", color: "#000", opacity: loading || !input.trim() ? .4 : 1 }}><IconSend/></motion.button>
            </div>
          </div>
        </>
      )}

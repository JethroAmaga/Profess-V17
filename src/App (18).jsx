...
function buildSVG(charOrKey, mood, isTalking) {
  ...
  const armL = `<rect x="18" y="132" rx="8" width="20" height="36" fill="${b}"/>
              <rect x="14" y="164" rx="6" width="18" height="28" fill="${b}"/>
              <ellipse cx="22" cy="196" rx="10" ry="8" fill="${s}"/>`;
  const armRBase = `<rect x="122" y="132" rx="8" width="20" height="36" fill="${b}"/>
                  <rect x="124" y="164" rx="6" width="18" height="28" fill="${b}"/>
                  <ellipse cx="138" cy="196" rx="10" ry="8" fill="${s}"/>`;
  ...
  const feetSVG = `<rect x="58" y="210" width="10" height="16" rx="3" fill="${darken(b,15)}"/>
                   <rect x="88" y="210" width="10" height="16" rx="3" fill="${darken(b,15)}"/>
                   <ellipse cx="65" cy="224" rx="14" ry="6" fill="${darken(b,20)}"/>
                   <ellipse cx="95" cy="224" rx="14" ry="6" fill="${darken(b,20)}"/>`;

  const chairBase = `<rect x="35" y="210" width="90" height="8" rx="2" fill="#1A1208"/>
                     <rect x="30" y="100" width="100" height="8" rx="3" fill="#1A1208"/>`;

  // Desk — shared by seated roles
  const desk = (deskColor = "#2A1E10") =>
    `<rect x="0" y="185" width="160" height="12" rx="2" fill="${deskColor}" stroke="#3A2A18" stroke-width="1"/>
     <rect x="0" y="197" width="160" height="40" fill="${darken(deskColor,6)}"/>`;
  ...
  if (isCoach) {
    ...
    switch(roleKey) {
      ...
      case "manager":
        sceneSVG = desk("#2A1E10") + `<rect x="20" y="178" rx="1" width="30" height="20" fill="#E8E0D0" opacity="0.5"/>`;
      ...
      case "defense_lawyer": case "prosecutor": case "cross_examiner":
        sceneSVG = desk("#2A1E10") +
          `<rect x="30" y="175" rx="2" width="36" height="20" fill="#E8DCC0" opacity=".9"/>
           <line x1="33" y1="180" x2="63" y2="180" stroke="#8A7A60" stroke-width="1"/>
           <line x1="33" y1="185" x2="63" y2="185" stroke="#8A7A60" stroke-width="1"/>`;
        bodySVG = `<rect x="40" y="128" rx="8" width="80" height="92" fill="${b}"/>
      ...
      case "dosen_pembimbing":
        sceneSVG = desk("#2A1E10") +
          `<rect x="25" y="175" rx="2" width="42" height="20" fill="#E8E0D0"/>
           <line x1="28" y1="180" x2="64" y2="180" stroke="#9A8A70" stroke-width="1"/>
           <line x1="28" y1="185" x2="64" y2="185" stroke="#9A8A70" stroke-width="1"/>
           <line x1="28" y1="190" x2="48" y2="190" stroke="#9A8A70" stroke-width="1"/>`;
      ...
      case "judge":
        sceneSVG = desk("#3A2810") +
          `<rect x="55" y="175" rx="2" width="50" height="20" fill="#E8E0CC"/>
           <rect x="95" y="167" rx="3" width="8" height="18" fill="#6A5030"/>
           <ellipse cx="99" cy="166" rx="8" ry="5" fill="#7A6040"/>`;
        bodySVG = `<rect x="40" y="128" rx="8" width="80" height="92" fill="${b}"/>
      ...
      case "ceo": case "executive": case "diplomat": case "acquirer":
        sceneSVG = desk("#2A2218") +
          `<rect x="20" y="178" rx="2" width="120" height="4" fill="#8A9080" opacity=".4"/>
           <rect x="30" y="108" width="100" height="60" rx="10" fill="#1A1610"/>`;
      ...
      case "date": case "blind_date": case "ex_partner":
        sceneSVG = `<rect x="20" y="190" width="120" height="35" rx="8" fill="#1E1408"/>
                    <rect x="15" y="145" width="5" height="50" rx="2" fill="#181008"/>
                    <rect x="140" y="145" width="5" height="50" rx="2" fill="#181008"/>`;
      ...
      case "calon_mertua": case "dosen_pembimbing":
        sceneSVG = `<rect x="10" y="148" width="140" height="72" rx="12" fill="#2A1E10"/>
                    <rect x="10" y="185" width="25" height="12" rx="4" fill="#221610"/>
                    <rect x="125" y="185" width="25" height="12" rx="4" fill="#221610"/>`;
  ...
  // Add chair seat/back for all seated roles
  const seatedRoles = new Set(["interviewer","reviewer","auditor","investigator","regulator","commissioner","official","customer_service","manager","defense_lawyer","prosecutor","cross_examiner","examiner","dean","professor_academic","panelist","dosen_pembimbing","judge","negotiator","partner","board_member","shareholder","ceo","executive","diplomat","acquirer","investor","client","customer","consumer","voter","vendor","contractor","friend_female","best_friend","crush","romantic_interest","date","blind_date","ex_partner","parent","grandparent","mentor","pak_rt","calon_mertua"]);
  if (seatedRoles.has(roleKey) || isCoach) sceneSVG = chairBase + sceneSVG;
...
const PLAYLISTS = [
  { id: "37i9dQZF1DWXjo7kaCGuCU", label: "Ambient", desc: "For formal sessions" },
  { id: "37i9dQZF1DWYoYGBbGKurt", label: "Lo-Fi",   desc: "For social sessions" },
...
export default function Profess() {
  ...
  const StepDots = () => stepIndex >= 0 ? (
    <div style={{ display:"flex", gap:"6px", alignItems:"center" }}>
      {ONBOARDING_STEPS.map((s,i) => (
        <div key={s} style={{ width: i===stepIndex?"20px":"5px", height:"3px", borderRadius:"2px", background: i<stepIndex?"#3A3530":i===stepIndex?"#C8A870":"#1E1E1E", transition:"all .3s ease" }}/>
      ))}
    </div>
  ) : null;
    ...
    return (
      ...
      <div className="screen-enter" style={{ ...BASE, overflowY:"auto", filter:warmMode?"sepia(0.18) saturate(1.2) brightness(1.02)":"none", transition:"filter 1.2s ease" }}>
        ...
        <div style={{ position:"relative", height:"100vh", display:"flex", flexDirection:"column", overflow:"hidden", paddingBottom:"0" }}>
          ...
          <div style={{ position:"absolute", right:0, top:0, height:"100%", width:"45%", opacity: warmMode ? 0.65 : 0.45, transition:"opacity 1.2s ease", pointerEvents:"none", zIndex:1 }}>
            <svg viewBox="0 0 100 100" preserveAspectRatio="xMidYMid slice" style={{ width:"100%", height:"100%" }}>
              <defs>
                <radialGradient id="lampGlow" cx="50%" cy="50%" r="50%">
                  <stop offset="0%" stopColor="#D4920A" stopOpacity="0.9"/>
                  <stop offset="55%" stopColor="#C87808" stopOpacity="0.4"/>
              ...
              <rect x="58" y="75" width="2.5" height="4" rx="0.5" fill="#3A2410"/>
              {/* Radio side table */}
              ...
              {/* Lamp glow (warmMode) */}
              {warmMode && <circle cx="50" cy="50" r="45" fill="url(#lampGlow)" opacity="0.7"/>}
              {/* Lamp shade */}
              <polygon points="44,59 56,59 53.5,51 46.5,51" fill={warmMode ? "#D4920A" : "#3A2A10"} stroke={warmMode ? "#E8A820" : "#5A4020"} strokeWidth="1.5"/>
              {/* Lamp base */}
              <rect x="47.5" y="59" width="5" height="9" rx="0.8" fill="#5A4020" stroke="#6A5030" strokeWidth="0.3"/>
              {/* Lamp foot */}
          ...
          <div style={{ position:"absolute", right:"28%", top:"42%", pointerEvents:"none", zIndex:6, textAlign:"center", transition:"color 1.2s ease" }}>
            <p style={{ fontFamily:"'Inter',sans-serif", fontWeight:300, fontSize:"9px", letterSpacing:".14em", textTransform:"uppercase", color:warmMode?"#C8A870":"#4A3828", opacity:0.85, whiteSpace:"nowrap", transition:"color 1.2s ease" }}>
              {warmMode ? "warm mode on" : "click the lamp"}
            </p>
            <span style={{ display:"block", width:"3px", height:"3px", borderRadius:"50%", background:warmMode?"#C8A870":"#4A3828", margin:"4px auto 0", animation:"pulse 1.5s ease-in-out infinite", transition:"background 1.2s ease" }}/>
          </div>
          {/* Lamp click overlay */}
          <div onClick={() => { setWarmMode(w => { const next = !w; setRoomMood(next ? "warm" : "neutral"); return next; }); }} title={warmMode ? "warm mode on" : "click the lamp"} style={{ position:"absolute", right:"22%", top:"51%", width:"6%", height:"16%", cursor:"pointer", zIndex:7, transform:"translateX(50%)" }}/>
          {/* Radio hint text */}
          <div style={{ position:"absolute", right:"8%", bottom:"30%", pointerEvents:"none", zIndex:6, textAlign:"center", transition:"color 0.8s ease" }}>
            <p style={{ fontFamily:"'Inter',sans-serif", fontWeight:300, fontSize:"8px", letterSpacing:".12em", textTransform:"uppercase", color:showPlayer?"#C8A870":"#3A3A42", opacity:showPlayer?0.9:0.75, whiteSpace:"nowrap", transition:"color 0.8s ease, opacity 0.8s ease" }}>
              {showPlayer ? "music on" : "press radio for music"}
            </p>
            <span style={{ display:"block", width:"3px", height:"3px", borderRadius:"50%", background:showPlayer?"#C8A870":"#3A3A42", margin:"4px auto 0", animation:"pulse 1.5s ease-in-out infinite", transition:"background 0.8s ease" }}/>
          </div>
          {/* Radio click overlay */}
          <div onClick={() => { hasOpenedMusic.current = true; setShowMusicSuggest(false); setShowPlayer(p => !p); }} title="Toggle music player" style={{ position:"absolute", right:"5%", top:"62%", width:"7%", height:"10%", cursor:"pointer", zIndex:7 }}/>

          {/* Hero content */}
          <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"flex-start", justifyContent:"center", padding:"80px 40px 0 clamp(40px,8vw,140px)", position:"relative", zIndex:2 }}>
            <div style={{ maxWidth:"640px", width:"100%", display:"flex", flexDirection:"column", gap:"24px" }}>
              <div>
                ...
                <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(44px,6vw,72px)", fontWeight:400, lineHeight:1.15, color:"#F0EDE6", letterSpacing:"-.01em" }}>
                  You cannot move people<br/>
                  you do not understand.
                </h1>
              </div>
              <div style={{ width:"100px", height:"1px", background:"#D4B47C" }}/>
              <p style={{ fontSize:"15px", lineHeight:1.8, color:"#5A5550", maxWidth:"480px" }}>
              ...
              <div>
                <button onClick={() => setScreen("lang")}
                  style={{ background:"none", border:"1px solid #3A3530", color:"#F0EDE6", fontFamily:"'Playfair Display',serif", fontSize:"14px", fontStyle:"italic", minWidth:"200px", height:"48px", display:"flex", alignItems:"center", justifyContent:"center", letterSpacing:".04em", transition:"background .25s, border-color .25s, color .25s" }}
                  ...
                  onMouseLeave={e=>{ e.currentTarget.style.background="none"; e.currentTarget.style.borderColor="#3A3530"; e.currentTarget.style.color="#F0EDE6"; }}>
                  Begin
                </button>
              </div>
            </div>
          ...
          {heroRow.map((charObj, i) => {
            ...
            return (
              <div key={i} style={{ position:"relative", flexShrink:0 }}
                ...
                {/* Tooltip */}
                <div style={{ position:"absolute", bottom:"100%", left:"50%", transform:"translateX(-50%)", marginBottom:"8px", background:"#0E0E0E", border:"1px solid #2A2520", padding:"10px 14px", opacity: hoveredChar === i ? 1 : 0, transition:"opacity 0.2s ease", pointerEvents:"none", zIndex:100, whiteSpace:"nowrap", textAlign:"center" }}>
                  <p style={{ fontFamily:"'Playfair Display',serif", fontSize:"13px", color: charObj.accent || "#C8A870", marginBottom:"4px" }}>{charObj.name || role}</p>
                  <p style={{ fontSize:"11px", color:"#5A5550", fontWeight:300, marginTop:"4px" }}>{heroRoleDesc(role)}</p>
                </div>
                <div style={{ width:"90px", height:"112px", opacity:0.85, animation: heroRoleAnims[i] }} dangerouslySetInnerHTML={{ __html: svg }}/>
              </div>
            );
          })}
        </div>
        ...
        <div style={{ position:"relative", background:"#060606", backgroundImage:"radial-gradient(circle, #1C1C1C 1px, transparent 1px)", backgroundSize:"24px 24px", borderTop:"1px solid #141414" }}>
          ...
          <div style={{ display:"flex", borderBottom:"1px solid #141414" }}>
            ...
            <button onClick={() => setScreen("lang")}
              ...
              <div style={{ position:"relative", width:"140px", marginBottom:"32px" }}>
              ...
              <div style={{ textAlign:"center" }}>
                <p style={{ fontFamily:"'Playfair Display',serif", fontSize:"13px", letterSpacing:".15em", color:"#C8A870", marginBottom:"8px" }}>BEGIN</p>
                <p style={{ fontSize:"12px", color:"#3A3530", fontWeight:300 }}>Start a session</p>
              </div>
            </button>
            ...
            <button onClick={() => setExpandedPanel(p => p==="about" ? null : "about")}
              ...
              <div style={{ textAlign:"center" }}>
                <p style={{ fontFamily:"'Playfair Display',serif", fontSize:"13px", letterSpacing:".15em", color:"#C8A870", marginBottom:"8px" }}>ABOUT</p>
                <p style={{ fontSize:"12px", color:"#3A3530", fontWeight:300 }}>What is Profess</p>
              </div>
            </button>
            ...
            <button onClick={() => setExpandedPanel(p => p==="terms" ? null : "terms")}
              ...
              <div style={{ textAlign:"center" }}>
                <p style={{ fontFamily:"'Playfair Display',serif", fontSize:"13px", letterSpacing:".15em", color:"#C8A870", marginBottom:"8px" }}>TERMS</p>
                <p style={{ fontSize:"12px", color:"#3A3530", fontWeight:300 }}>Terms & Conditions</p>
              </div>
            </button>
          </div>
              ...
              <div style={{ maxWidth:"680px", margin:"0 auto", display:"flex", flexDirection:"column", gap:"0" }}>
                {TERMS_SECTIONS.map((item,i) => (
                  <div key={i} style={{ paddingBottom:"28px", marginBottom:"28px", borderBottom: i < TERMS_SECTIONS.length-1 ? "1px solid #0E0E0E" : "none" }}>
                    ...
                    <p style={{ fontSize:"13px", lineHeight:1.9, color:"#5A5550", fontWeight:300 }}>{item.body}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div onClick={() => { hasOpenedMusic.current = true; setShowPlayer(p => !p); }} onMouseEnter={e => e.currentTarget.style.background="#141410"} onMouseLeave={e => e.currentTarget.style.background="#0A0A0A"} style={{ position:"fixed", bottom:0, left:0, zIndex:490, width:"48px", height:"48px", background:"#0A0A0A", borderTop:`1px solid ${showPlayer?"#C8A870":"#1E1E1E"}`, borderRight:`1px solid ${showPlayer?"#C8A870":"#1E1E1E"}`, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", transition:"background 0.2s ease" }}>
        <svg width="24" height="18" viewBox="0 0 24 18" fill="none"><rect x="1" y="5" width="17" height="11" rx="1.5" fill="#4A4A52" stroke="#5A5A62" strokeWidth="1"/><line x1="3" y1="8" x2="12" y2="8" stroke="#3A3A42" strokeWidth="0.8" opacity="0.9"/><line x1="3" y1="11" x2="12" y2="11" stroke="#3A3A42" strokeWidth="0.8" opacity="0.9"/><circle cx="16" cy="8.5" r="2" fill="#6A6A72"/><line x1="19" y1="5" x2="23" y2="0.5" stroke={showPlayer?"#C8A870":"#5A5A62"} strokeWidth="1" style={showPlayer?{animation:"antennaP 1s ease-in-out infinite"}:{opacity:0.6}}/></svg>
      </div>
      </>
    );
  }
  ...
  if (screen === "lang") return (
    <div className="screen-enter" style={{ ...BASE, display:"flex", height:"100vh", overflow:"hidden", position:"relative" }}>
      ...
      <div style={{ position:"fixed", top:"24px", left:"40px", zIndex:50 }}>
        <button onClick={() => setScreen("landing")} style={{ background:"none", border:"none", color:"#3A3530", fontSize:"10px", letterSpacing:".1em", textTransform:"uppercase", display:"flex", alignItems:"center", gap:"5px", transition:"color .2s", padding:0, cursor:"pointer" }}
          onMouseEnter={e=>e.currentTarget.style.color="#6A6560"} onMouseLeave={e=>e.currentTarget.style.color="#3A3530"}>
      ...
      {[
        ...
        <button key={opt.code}
          ...
          <div style={{ position:"relative", zIndex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:"16px" }}>
            <p style={{ fontSize:"9px", letterSpacing:".15em", textTransform:"uppercase", color:"#3A3530", marginBottom:"8px" }}>{opt.label}</p>
            ...
            <p style={{ fontSize:"13px", color:"#4A4540", letterSpacing:".03em" }}>{opt.sub}</p>
          </div>
        </button>
      ))}
      {showPlayer && <MusicWidget activePlaylist={activePlaylist} setActivePlaylist={setActivePlaylist} onClose={() => setShowPlayer(false)}/>}
  ...
  if (screen === "mode") return (
    <div className="screen-enter" style={{ ...BASE, display:"flex", height:"100vh", overflow:"hidden", position:"relative" }}>
      ...
      {/* Back + steps */}
      <div style={{ position:"fixed", top:"24px", left:"40px", zIndex:50 }}>
        <button onClick={() => setScreen("lang")} style={{ background:"none", border:"none", color:"#3A3530", fontSize:"10px", letterSpacing:".1em", textTransform:"uppercase", display:"flex", alignItems:"center", gap:"5px", transition:"color .2s", padding:0, cursor:"pointer" }}
          onMouseEnter={e=>e.currentTarget.style.color="#6A6560"} onMouseLeave={e=>e.currentTarget.style.color="#3A3530"}>
      ...
      {[
        ...
        <button key={opt.key}
          ...
          <div style={{ position:"relative", zIndex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:"16px", maxWidth:"320px" }}>
            ...
            <p style={{ fontSize:"13px", color:"#4A4540", lineHeight:1.7, textAlign:"center", letterSpacing:".03em" }}>{opt.desc}</p>
          </div>
        </button>
      ))}
      {showPlayer && <MusicWidget activePlaylist={activePlaylist} setActivePlaylist={setActivePlaylist} onClose={() => setShowPlayer(false)}/>}
      ...
      <div className="screen-enter" style={{ ...BASE, display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ position:"relative", zIndex:1, padding:"0 40px", height:"56px", borderBottom:"1px solid #141414", display:"flex", alignItems:"center", gap:"12px" }}>
          ...
          <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:"20px" }}>
            <StepDots/>
            <button onClick={() => setScreen("mode")} style={{ background:"none", border:"none", color:"#3A3530", fontSize:"10px", letterSpacing:".1em", textTransform:"uppercase", display:"flex", alignItems:"center", gap:"5px", transition:"color .2s", padding:0, cursor:"pointer" }}
              onMouseEnter={e=>e.currentTarget.style.color="#6A6560"} onMouseLeave={e=>e.currentTarget.style.color="#3A3530"}>
        ...
        <div style={{ position:"relative", zIndex:1, flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:"48px 24px" }}>
          <div style={{ maxWidth:"560px", width:"100%" }}>
            <div style={{ marginBottom:"40px" }}>
              <p style={{ fontSize:"9px", letterSpacing:".15em", textTransform:"uppercase", color:"#3A3530", marginBottom:"12px" }}>{isFormal?(isID?"Formal":"Formal"):(isID?"Sosial":"Social")}</p>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:"16px", marginBottom:"48px" }}>
              ...
              {dc.body.slice(1).map((para,i) => (
                <p key={i} style={{ fontSize:"13px", lineHeight:1.9, color:"#4A4540", letterSpacing:".03em" }}>{para}</p>
              ))}
            </div>
    ...
    return (
      ...
      <div className="screen-enter" style={{ ...BASE, display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ position:"relative", zIndex:1, padding:"0 40px", height:"56px", borderBottom:"1px solid #141414", display:"flex", alignItems:"center" }}>
          ...
          <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:"20px" }}>
            <StepDots/>
            <button onClick={() => setScreen("disclaimer")} style={{ background:"none", border:"none", color:"#3A3530", fontSize:"10px", letterSpacing:".1em", textTransform:"uppercase", display:"flex", alignItems:"center", gap:"5px", transition:"color .2s", padding:0, cursor:"pointer" }}
              onMouseEnter={e=>e.currentTarget.style.color="#6A6560"} onMouseLeave={e=>e.currentTarget.style.color="#3A3530"}>
              <IconArrowLeft/> {isID?"Kembali":"Back"}
            </button>
          </div>
        ...
        <div style={{ position:"relative", zIndex:1, flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:"40px 24px" }}>
          <div style={{ width:"100%", maxWidth:"480px" }}>
            <div style={{ marginBottom:"40px" }}>
              <p style={{ fontSize:"9px", letterSpacing:".15em", textTransform:"uppercase", color:"#3A3530", marginBottom:"12px" }}>{isID?"Intensitas":"Intensity"}</p>
              ...
              {levels.map((lvl, idx) => (
                <button key={lvl.key}
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:"13px", color:"#4A4540", lineHeight:1.7, maxWidth:"320px", letterSpacing:".03em" }}>{lvl.desc}</div>
                  </div>
                </button>
              ))}
            </div>
    ...
    return (
      <div className="screen-enter" style={{ ...BASE, display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ position:"relative", zIndex:1, padding:"0 40px", height:"56px", borderBottom:"1px solid #141414", display:"flex", alignItems:"center", flexShrink:0 }}>
          ...
          <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:"20px" }}>
            <StepDots/>
            <button onClick={() => setScreen("intensity")} style={{ background:"none", border:"none", color:"#3A3530", fontSize:"10px", letterSpacing:".1em", textTransform:"uppercase", display:"flex", alignItems:"center", gap:"5px", transition:"color .2s", padding:0, cursor:"pointer" }}
              onMouseEnter={e=>e.currentTarget.style.color="#6A6560"} onMouseLeave={e=>e.currentTarget.style.color="#3A3530"}>
              <IconArrowLeft/> {isID?"Kembali":"Back"}
            </button>
          </div>
        ...
        <div style={{ position:"relative", zIndex:1, flex:1, overflowY:"auto", padding:"40px" }}>
          <div style={{ maxWidth:"600px", margin:"0 auto" }}>
            <div style={{ marginBottom:"40px" }}>
              ...
              <p style={{ fontSize:"13px", color:"#4A4540", letterSpacing:".03em" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:"40px" }}>
              {groups.map((group, gi) => (
                <div key={gi}>
                  <p style={{ fontSize:"9px", color:"#3A3530", letterSpacing:".15em", textTransform:"uppercase", marginBottom:"16px", paddingBottom:"8px", borderBottom:"1px solid #141414" }}>
                    {group.group}
                  </p>
      ...
      <div className="screen-enter" style={{ ...BASE, display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:"48px 24px" }}>
          <div style={{ maxWidth:"520px", width:"100%" }}>
            <p style={{ fontSize:"9px", letterSpacing:".15em", textTransform:"uppercase", color:"#3A3530", marginBottom:"12px" }}>
            ...
            <div style={{ display:"flex", gap:"12px" }}>
              ...
              <button onClick={resetSession}
                style={{ background:"none", border:"1px solid #1E1E1E", color:"#4A4540", fontFamily:"inherit", fontSize:"10px", padding:"10px 24px", letterSpacing:".12em", textTransform:"uppercase", transition:"border-color .2s, color .2s", cursor:"pointer" }}
                onMouseEnter={e=>{ e.currentTarget.style.borderColor="#3A3530"; e.currentTarget.style.color="#8A8580"; }}
                onMouseLeave={e=>{ e.currentTarget.style.borderColor="#1E1E1E"; e.currentTarget.style.color="#4A4540"; }}>
  ...
  const msgList = (fontSize="15px") => messages.map((msg,i) => {
    ...
    return (
      <div key={i} className="msg-enter" style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
        <span style={{ fontSize:"9px", letterSpacing:".12em", textTransform:"uppercase", fontWeight:500, color:isA?(mInRole?mc.accent:"#3A3530"):"#2A2A2A" }}>
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
      {showMusicSuggest && (
        <button onClick={() => { hasOpenedMusic.current = true; setShowMusicSuggest(false); setActivePlaylist(sessionMode === "social" ? 1 : 0); setShowPlayer(true); }}
          style={{ background:"none", border:"none", color:"#3A3530", fontFamily:"inherit", fontSize:"10px", letterSpacing:".06em", padding:0, textAlign:"left", cursor:"pointer", animation:"musicFadeIn .4s ease forwards", transition:"color .2s" }}
      ...
      <div style={{ display:"flex", gap:"8px", alignItems:"flex-end" }}>
        ...
        <button onClick={toggleMic} className={isListening?"mic-active":""}
          style={{ background:isListening?"#180C0C":"none", border:`1px solid ${isListening?"#7A4040":"#1A1A1A"}`, color:isListening?"#BC7A7A":"#3A3530", width:btnSz, height:btnSz, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, transition:"all .2s" }}
          onMouseEnter={e=>{ if(!isListening){e.currentTarget.style.borderColor="#2A2520";e.currentTarget.style.color="#6A6560";} }}
          onMouseLeave={e=>{ if(!isListening){e.currentTarget.style.borderColor="#1A1A1A";e.currentTarget.style.color="#3A3530";} }}>
  ...
  const actionRow = (compact=false) => (
    <div style={{ display:"flex", gap:"16px", alignItems:"center", marginBottom:compact?"6px":"8px" }}>
      ...
      <button onClick={endSession} style={{ background:"none", border:"none", color:"#2A2520", fontFamily:"inherit", fontSize:"10px", letterSpacing:".08em", textTransform:"uppercase", marginLeft:"auto", transition:"color .2s", padding:0 }}
  ...
  return (
    <div style={{ ...BASE, display:"flex", flexDirection:"column" }}>
      ...
      {/* Session header */}
      <div style={{ padding:`0 ${isMobile?"16px":"32px"}`, height:isMobile?"48px":"56px", borderBottom:"1px solid #141414", display:"flex", alignItems:"center", gap:"16px", flexShrink:0, position:"relative" }}>
        ...
        {!isMobile && <>
          ...
          <span style={{ fontSize:"9px", color:"#2A2520", letterSpacing:".12em", textTransform:"uppercase" }}>{sessionMode}</span>
          {intensityMeta()}
        </>}
        {/* Centered pills */}
        <div style={{ position:"absolute", left:"50%", transform:"translateX(-50%)", display:"flex", gap:"8px", alignItems:"center" }}>
          <div onClick={() => { hasOpenedMusic.current = true; setShowPlayer(p => !p); }} style={{ padding:"4px 12px", background:"none", border:`1px solid ${showPlayer?"#C8A870":"#2A2520"}`, borderRadius:"12px", display:"flex", gap:"6px", alignItems:"center", cursor:"pointer", transition:"border-color 0.2s" }}>
            <svg width="16" height="12" viewBox="0 0 16 12" fill="none">
              <rect x="0.5" y="2" width="11" height="8" rx="1" fill="#4A4A52"/>
              <line x1="2" y1="4.5" x2="8" y2="4.5" stroke="#3A3A42" strokeWidth="0.7"/>
              <line x1="2" y1="7" x2="8" y2="7" stroke="#3A3A42" strokeWidth="0.7"/>
              <circle cx="11" cy="5" r="1.2" fill="#6A6A72"/>
              <line x1="13" y1="2" x2="15.5" y2="0" stroke={showPlayer?"#C8A870":"#5A5A62"} strokeWidth="0.8" style={showPlayer ? {animation:"antennaP 1s ease-in-out infinite"} : {opacity:0.5}}/>
            </svg>
            <span style={{ fontFamily:"'Inter',sans-serif", fontWeight:300, fontSize:"9px", letterSpacing:"0.08em", color:showPlayer?"#C8A870":"#4A4540", transition:"color 0.2s" }}>music</span>
          </div>
          <div onClick={() => { setWarmMode(w => { const next = !w; setRoomMood(next ? "warm" : "neutral"); return next; }); }} style={{ padding:"4px 12px", background:"none", border:`1px solid ${warmMode?"#C8A870":"#2A2520"}`, borderRadius:"12px", display:"flex", gap:"6px", alignItems:"center", cursor:"pointer", transition:"border-color 0.2s" }}>
            <svg width="10" height="14" viewBox="0 0 10 14" fill="none">
              <polygon points="1,7 9,7 7.5,2 2.5,2" fill={warmMode?"#D4920A":"#3A2A10"} stroke={warmMode?"#E8A820":"#4A3A20"} strokeWidth="0.8"/>
              <line x1="5" y1="7" x2="5" y2="12" stroke={warmMode?"#E8A820":"#4A3A20"} strokeWidth="1.2"/>
            </svg>
            <span style={{ fontFamily:"'Inter',sans-serif", fontWeight:300, fontSize:"9px", letterSpacing:"0.08em", color:warmMode?"#C8A870":"#4A4540", transition:"color 0.2s" }}>warm</span>
          </div>
        </div>
        <div style={{ marginLeft:"auto", display:"flex", gap:"8px", alignItems:"center" }}>
          <button onClick={()=>setIsMobile(p=>!p)} style={{ background:"none", border:"1px solid #1A1A1A", color:"#2A2520", padding:"6px 8px", display:"flex", alignItems:"center", justifyContent:"center", transition:"border-color .2s, color .2s" }}
            onMouseEnter={e=>{ e.currentTarget.style.borderColor="#2A2520"; e.currentTarget.style.color="#5A5550"; }}
            onMouseLeave={e=>{ e.currentTarget.style.borderColor="#1A1A1A"; e.currentTarget.style.color="#2A2520"; }}>
          ...
          <button onClick={resetSession} style={{ background:"none", border:"1px solid #1A1A1A", color:"#2A2520", fontFamily:"inherit", fontSize:"9px", padding:"6px 14px", letterSpacing:".1em", textTransform:"uppercase", transition:"border-color .2s, color .2s" }}
            onMouseEnter={e=>{ e.currentTarget.style.borderColor="#2A2520"; e.currentTarget.style.color="#5A5550"; }}
            onMouseLeave={e=>{ e.currentTarget.style.borderColor="#1A1A1A"; e.currentTarget.style.color="#2A2520"; }}>
            {lang==="id"?"Baru":"New"}
          </button>
      ...
      {warmMode && <div style={{ position:"fixed", inset:0, pointerEvents:"none", zIndex:5, background:"radial-gradient(ellipse at 20% 80%, rgba(180,100,20,0.08) 0%, transparent 60%)" }}/>}
      ...
      {isMobile ? (
        <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
          ...
          <div style={{ background:isTransitioning?"#080808":charMeta.bg, transition:"background .4s ease", borderBottom:"1px solid #141414", padding:"10px 16px", display:"flex", alignItems:"center", gap:"14px", flexShrink:0 }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ fontSize:"9px", color:"#3A3530", letterSpacing:".1em", textTransform:"uppercase" }}>{charMeta.title}</div>
              {currentMood !== "neutral" && (
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"24px 16px 12px", display:"flex", flexDirection:"column", gap:"28px" }}>
            {msgList("15px")}
            {loading && loadingDots()}
        ...
        <div style={{ flex:1, display:"flex", overflow:"hidden" }}>
          ...
          <div style={{ width:"240px", flexShrink:0, borderRight:"1px solid #141414", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"flex-start", background:isTransitioning?"#080808":`radial-gradient(ellipse at 50% 40%, ${charMeta.bg} 0%, #080808 75%)`, transition:"background .5s ease", position:"fixed", top:"56px", left:0, height:"calc(100vh - 56px)", overflow:"hidden", zIndex:10 }}>
            <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"16px", padding:"32px 16px 24px", width:"100%" }}>
              ...
              <div style={{ textAlign:"center", opacity:isTransitioning?0:1, transition:"opacity .38s", width:"100%" }}>
                ...
                <div style={{ fontSize:"9px", color:"#3A3530", letterSpacing:".1em", textTransform:"uppercase", lineHeight:1.6 }}>{charMeta.title}</div>
                {currentMood !== "neutral" && (
          ...
          <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", marginLeft:"240px" }}>
            <div style={{ flex:1, overflowY:"auto", padding:"32px 32px 16px", display:"flex", flexDirection:"column", gap:"28px", alignItems:"stretch" }}>
...
function MusicWidget({ activePlaylist, setActivePlaylist, onClose }) {
  return (
    <div style={{ position:"fixed", bottom:0, left:0, right:"auto", zIndex:500, background:"#0A0A0A", border:"1px solid #2A2520", borderRadius:"0 8px 0 0", width:"300px", boxShadow:"0 8px 32px rgba(0,0,0,0.6)" }}>
      {/* Header */}
      <div style={{ padding:"8px 12px", borderBottom:"1px solid #141414", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <span style={{ fontSize:"9px", letterSpacing:".12em", textTransform:"uppercase", color:"#3A3530" }}>Now Playing</span>
        <button onClick={onClose} style={{ background:"none", border:"none", color:"#4A4540", fontSize:"16px", lineHeight:1, cursor:"pointer", padding:0 }}>×</button>
      </div>
      {/* Playlist selector */}
      ...
      {/* Spotify embed */}
      <iframe
        src={`https://open.spotify.com/embed/playlist/${PLAYLISTS[activePlaylist].id}?utm_source=generator&theme=0`}

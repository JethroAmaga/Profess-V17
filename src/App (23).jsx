...
function buildSVG(charOrKey, mood, isTalking) {
    ...
    switch(roleKey) {
      ...
      case "blind_date": case "colleague": case "stranger":
        ...
        frontProps = `
          ...
          <rect x="54" y="208" width="14" height="22" rx="5" fill="${darken(b,18)}"/>
          <rect x="92" y="208" width="14" height="22" rx="5" fill="${darken(b,18)}"/>
  ...
  // Landing-page decorative characters render with no role-based furniture/props
        ...
        <div style={{ position:"relative", height:"100vh", display:"flex", flexDirection:"column", overflow:"hidden", paddingBottom:"0" }}>
          ...
          <div style={{ position:"absolute", right:0, top:0, height:"100%", width:"45%", opacity: warmMode ? 0.55 : 0.35, transition:"opacity 1.2s ease", pointerEvents:"none", zIndex:1 }}>
            <svg viewBox="0 0 100 100" preserveAspectRatio="xMidYMid slice" style={{ width:"100%", height:"100%" }}>
              ...
              {/* Left sofa — 3/4 perspective (recedes toward center-right) */}
              {/* Backrest — front face */}
              <polygon points="5,52 33,52 33,64 5,64" fill="#221408" stroke="#2A1A0A" strokeWidth="0.25"/>
              {/* Backrest — top face (thickness) */}
              <polygon points="5,52 33,52 39,48 11,48" fill="#2C1C0C" stroke="#2A1A0A" strokeWidth="0.25"/>
              {/* Backrest — slanted side face (depth) */}
              <polygon points="33,52 39,48 39,60 33,64" fill="#160E06" stroke="#2A1A0A" strokeWidth="0.25"/>
              {/* Seat — front face */}
              <polygon points="5,64 37,64 37,75 5,75" fill="#1A120A" stroke="#2A1A0A" strokeWidth="0.25"/>
              {/* Seat — top face (cushion surface) */}
              <polygon points="5,64 37,64 43,60 11,60" fill="#251808" stroke="#2A1A0A" strokeWidth="0.25"/>
              {/* Seat — slanted side face (depth) */}
              <polygon points="37,64 43,60 43,71 37,75" fill="#120B05" stroke="#2A1A0A" strokeWidth="0.25"/>
              {/* Cushion seams on top face */}
              <line x1="20" y1="60.6" x2="22.7" y2="64" stroke="#1A1006" strokeWidth="0.4" opacity="0.7"/>
              {/* Armrest — near corner */}
              <polygon points="3,58 9.5,58 9.5,75 3,75" fill="#221408" stroke="#2A1A0A" strokeWidth="0.25"/>
              <polygon points="3,58 9.5,58 11,55.5 4.5,55.5" fill="#2C1C0C"/>
              {/* Left character */}
              <foreignObject x="9" y="40" width="22" height="34" overflow="visible" opacity={warmMode ? 0.75 : 0.45}>
              ...
              {/* Right sofa — 3/4 perspective (recedes toward center-left) */}
              {/* Backrest — front face */}
              <polygon points="61,54 89,54 89,65 61,65" fill="#181008" stroke="#221208" strokeWidth="0.25"/>
              {/* Backrest — top face (thickness) */}
              <polygon points="61,54 89,54 83,50 55,50" fill="#221808"/>
              {/* Backrest — slanted side face (depth, on inner/left edge) */}
              <polygon points="61,54 55,50 55,62 61,65" fill="#120C06" stroke="#221208" strokeWidth="0.25"/>
              {/* Seat — front face */}
              <polygon points="59,65 91,65 91,75 59,75" fill="#181008" stroke="#221208" strokeWidth="0.25"/>
              {/* Seat — top face (cushion surface) */}
              <polygon points="59,65 91,65 85,61 53,61" fill="#201608"/>
              {/* Seat — slanted side face (depth, inner/left edge) */}
              <polygon points="59,65 53,61 53,71 59,75" fill="#100A05" stroke="#221208" strokeWidth="0.25"/>
              {/* Cushion seam on top face */}
              <line x1="74" y1="61.4" x2="71.6" y2="65" stroke="#160E06" strokeWidth="0.4" opacity="0.7"/>
              {/* Armrest — near corner */}
              <polygon points="87.5,58 94,58 94,75 87.5,75" fill="#181008" stroke="#221208" strokeWidth="0.25"/>
              <polygon points="87.5,58 94,58 89.5,55 84,55" fill="#221808"/>
              {/* Right character */}
              <foreignObject x="64" y="42" width="20" height="32" overflow="visible" opacity={warmMode ? 0.75 : 0.45}>
          ...
          {/* Radio hint text */}
          <div onClick={() => { hasOpenedMusic.current = true; setShowMusicSuggest(false); setShowPlayer(p => !p); }} title="Toggle music player" style={{ position:"absolute", right:"8%", bottom:"30%", pointerEvents:"all", cursor:"pointer", zIndex:8, textAlign:"center", transition:"color 0.8s ease" }}>
          ...
          <div onClick={() => { hasOpenedMusic.current = true; setShowMusicSuggest(false); setShowPlayer(p => !p); }} title="Toggle music player" style={{ position:"absolute", right:"4%", top:"60%", width:"9%", height:"14%", cursor:"pointer", zIndex:8 }}/>
    ...
    <div style={{ ...BASE, display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ padding:`0 ${isMobile?"16px":"32px"}`, height:isMobile?"48px":"56px", borderBottom:"1px solid #141414", display:"flex", alignItems:"center", gap:"16px", flexShrink:0 }}>
        ...
        <div style={{ flex:1, display:"flex", justifyContent:"center", alignItems:"center", gap:"10px" }}>
          <button onClick={() => { hasOpenedMusic.current = true; setShowPlayer(p => !p); }} title="Music" style={{ background:"none", border:"1px solid #1A1A1A", borderRadius:"999px", color:showPlayer?sessionAccent:"#2A2520", padding:"5px 12px", display:"flex", alignItems:"center", gap:"6px", cursor:"pointer", transition:"border-color .2s, color .2s" }}
          ...
          <button onClick={() => setWarmMode(w => !w)} title={warmMode ? "Warm mode on" : "Warm mode off"} style={{ background:"none", border:"1px solid #1A1A1A", borderRadius:"999px", color:warmMode?"#C8A458":"#2A2520", padding:"5px 12px", display:"flex", alignItems:"center", gap:"6px", cursor:"pointer", transition:"border-color .2s, color .2s" }}
            onMouseEnter={e=>{ e.currentTarget.style.borderColor="#2A2520"; }}
            onMouseLeave={e=>{ e.currentTarget.style.borderColor="#1A1A1A"; }}>
            <IconLamp/>
        ...
        <div style={{ display:"flex", gap:"8px", alignItems:"center" }}>
          <button onClick={()=>setIsMobile(p=>!p)} style={{ background:"none", border:"1px solid #1A1A1A", color:"#2A2520", padding:"6px 8px", display:"flex", alignItems:"center", justifyContent:"center", transition:"border-color .2s, color .2s" }}
            onMouseEnter={e=>{ e.currentTarget.style.borderColor="#2A2520"; e.currentTarget.style.color="#C8A458"; }}

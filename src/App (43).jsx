...
import { warmOverlayStyle } from "./warmMode.js";
...
export default function Profess() {
  ...
  // Presence mood: what the rig visibly shows right now. Overrides the AI's last [MOOD] tag
  // while a reply is generating (thinking) or the user is mid-turn (listening), so the
  // character never sits inert during exactly the moments the user is watching it most.
  ...
  const css = `
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    @keyframes listenSettle{0%,100%{transform:translateY(0) rotate(0deg)}45%{transform:translateY(-1px) rotate(0.8deg)}70%{transform:translateY(-0.5px) rotate(-0.4deg)}}
  ...
  if (screen === "landing") {
    ...
    return (
      ...
      <div className="screen-enter" style={{ ...BASE, overflowY:"auto" }}>
        ...
        {/* Warm mode: light source + ambient pacing shift — not a color filter. The character (roomMood→buildSVG) and the room light move together; nothing is overlaid on top of a screenshot. */}
        <div style={{ ...warmOverlayStyle(warmMode, { position:"65% 40%", fade:"80%", transitionMs:2200, fixed:true }), zIndex:998 }}/>
  ...
  />
          ...
          <div className="hero-content" style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"flex-start", justifyContent:"center", padding:"80px 40px 0 clamp(40px,8vw,140px)", position:"relative", zIndex:2 }}>
            <div style={{ maxWidth:"640px", width:"100%", display:"flex", flexDirection:"column", gap:"24px" }}>
              <div>
                <p style={{ fontSize:"9px", letterSpacing:".24em", textTransform:"uppercase", color:"#5A5248", marginBottom:"18px" }}>Communication Training</p>
                <h1 className="hero-tagline" style={{ fontFamily:"'Playfair Display',serif", fontStyle:"italic", fontSize:"clamp(42px,5.4vw,66px)", fontWeight:400, lineHeight:1.18, color:"#F0EDE6", letterSpacing:"0" }}>
...
>
      ...
      {[
        ...
        <button key={opt.code}
          ...
          <div style={{ position:"relative", zIndex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:"16px" }}>
            ...
            <h2 style={{ fontFamily:"'Playfair Display',serif", fontStyle:"italic", fontSize:"clamp(40px,5vw,64px)", fontWeight:400, color:opt.accent, lineHeight:1.1 }}>{opt.name}</h2>
      ...
      {[
        ...
        <button key={opt.key}
          ...
          <div style={{ position:"relative", zIndex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:"16px", maxWidth:"320px" }}>
            <h2 style={{ fontFamily:"'Playfair Display',serif", fontStyle:"italic", fontSize:"clamp(36px,4.5vw,56px)", fontWeight:400, color:opt.accent, lineHeight:1.1 }}>{opt.name}</h2>
        ...
        <div style={{ position:"relative", zIndex:1, flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:"48px 24px" }}>
          <div style={{ maxWidth:"560px", width:"100%" }}>
            <div style={{ marginBottom:"40px" }}>
              ...
              <h2 style={{ fontFamily:"'Playfair Display',serif", fontStyle:"italic", fontSize:"32px", fontWeight:400, color:"#D8D5CE", lineHeight:1.2 }}>{dc.title}</h2>
            </div>
        ...
        <div style={{ position:"relative", zIndex:1, flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:"40px 24px" }}>
          <div style={{ width:"100%", maxWidth:"480px" }}>
            <div style={{ marginBottom:"40px" }}>
              ...
              <h2 style={{ fontFamily:"'Playfair Display',serif", fontStyle:"italic", fontSize:"28px", fontWeight:400, color:"#D8D5CE" }}>{isID?"Seberapa keras Profess mendorongmu":"How hard should Profess push you"}</h2>
            </div>
  ...
  const charAnimBlock = (size) => {
    ...
    const moodAnim = getMoodAnimation(presenceMood, isTalking);
    const talkAnim = isTalking && isSpeaking ? "talkBody .4s ease-in-out infinite" : null;
    const AURA_COLORS = { warm:"#C8A870", amused:"#C8A870", surprised:"#A8B8D0", thinking:"#7A8AA0", skeptical:"#8A7A60", serious:"#4A4040", uncomfortable:"#8A6060", listening:"#8AA08A" };
    const moodColor = AURA_COLORS[presenceMood];
    const w = size; const h = isLarge ? "250px" : "72px";
    // Breathing runs on its own wrapper so it never gets clobbered by the mood/talk animation —
    // both target `transform`, and a single element can only run one transform animation at a
    // time. Nesting them lets the motions compose instead of one replacing the other, so the
    // character keeps breathing even while thinking, listening, or mid-sentence.
    const charEl = (
      <div key={displayRole} style={{ width:"100%", height:"100%", animation:idleAnim, position:"relative" }}>
        <div key={presenceMood+isTalking} className="char-enter" style={{ width:"100%", height:"100%", opacity:isTransitioning?0:1, transition:"opacity .38s", animation:talkAnim||moodAnim||undefined, filter:isSpeaking?`drop-shadow(0 0 ${isLarge?"18px":"8px"} ${charMeta.accent}40)`:"none", position:"relative", zIndex:1 }} dangerouslySetInnerHTML={{ __html:charSVG }}/>
      </div>
    );
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
          ...
          <div style={{ background:isTransitioning?"#080808":charMeta.bg, transition:"background .4s ease", borderBottom:"1px solid #141414", padding:"12px 16px", display:"flex", alignItems:"center", gap:"14px", flexShrink:0 }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontFamily:"'Playfair Display',serif", fontStyle:"italic", fontSize:"17px", color:charMeta.accent, marginBottom:"2px" }}>{charMeta.name}</div>
              <div style={{ fontSize:"10px", color:"#C8A458", letterSpacing:".1em", textTransform:"uppercase" }}>{charMeta.title}</div>
              {presenceMood !== "neutral" && (
                <div style={{ marginTop:"5px", display:"inline-flex", alignItems:"center", gap:"5px" }}>
                  ...
                  <span style={{ fontSize:"8px", color:charMeta.accent, letterSpacing:".1em", textTransform:"uppercase", opacity:.8 }}>{presenceMood}</span>
                </div>
              )}
            </div>
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"24px 16px 12px", display:"flex", flexDirection:"column", gap:"28px", position:"relative" }}>
            <div style={warmOverlayStyle(warmMode, { position:"50% 20%", fade:"75%", transitionMs:1000 })}/>
            {msgList("15px")}
            {loading && loadingDots()}
        ...
        <div style={{ flex:1, display:"flex", overflow:"hidden" }}>
          ...
          <div style={{ width:"240px", flexShrink:0, borderRight:"1px solid #141414", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"flex-start", background:isTransitioning?"#080808":`radial-gradient(ellipse at 50% 40%, ${charMeta.bg} 0%, #080808 75%)`, transition:"background .5s ease", position:"fixed", top:"56px", left:0, height:"calc(100vh - 56px)", overflow:"hidden", zIndex:10 }}>
            <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"16px", padding:"32px 16px 24px", width:"100%" }}>
              ...
              <div style={{ textAlign:"center", opacity:isTransitioning?0:1, transition:"opacity .38s", width:"100%" }}>
                <div style={{ fontFamily:"'Playfair Display',serif", fontStyle:"italic", fontSize:"18px", color:charMeta.accent, marginBottom:"6px" }}>{charMeta.name}</div>
                ...
                {presenceMood !== "neutral" && (
                  <div style={{ marginTop:"12px", display:"inline-flex", alignItems:"center", gap:"7px" }}>
                    ...
                    <span style={{ fontSize:"9px", color:charMeta.accent, letterSpacing:".12em", textTransform:"uppercase", opacity:.75 }}>{presenceMood}</span>
                  </div>
                )}
              </div>
          ...
          <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", marginLeft:"240px" }}>
            <div style={{ flex:1, overflowY:"auto", padding:"32px 32px 16px", display:"flex", flexDirection:"column", gap:"28px", alignItems:"stretch", position:"relative" }}>
              <div style={warmOverlayStyle(warmMode, { position:"50% 15%", fade:"75%", transitionMs:1000 })}/>

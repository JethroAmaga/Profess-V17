...
export default function Profess() {
  ...
  function Overlay({ children, footer, width = "min(520px, 90vw)", align = "left", placement = "left", panel = false }) {
    ...
    // Color tracks the active mode's accent family — warm gold for formal,
    // deep maroon for social — never the bright accent itself (too loud as a
    // large fill), and padding/overflow are owned by the card so text can
    // never spill past its edge.
    const isSocial = (sessionMode || pendingMode) === "social";
    const panelBg = isSocial ? "#2A1414" : "#241B0E";
    const panelBorder = isSocial ? "#4A222088" : "#4A360F88";
    return (
          ...
          <AnimatePresence>
            {transcriptOpen && (
              <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }}
                transition={{ duration: 0.3, ease: [0.22,1,0.36,1] }}
                style={{ position: "absolute", left: isMobile ? "20px" : "56px", top: isMobile ? "calc(4% + 28dvh + 80px)" : "72px", bottom: isMobile ? "20px" : "150px",
                  width: isMobile ? "88vw" : "min(42%, 520px)", maxWidth: isMobile ? "88vw" : "60%", right: isMobile ? "auto" : "34%",
                  padding: "20px 24px", overflowY: "auto", zIndex: 8, boxSizing: "border-box", borderRadius: "18px",

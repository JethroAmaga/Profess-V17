...
export default function Profess() {
  ...
  const ChoiceRow = ({ onClick, accent, children, sub, bars, photoSeed }) => (
    <motion.button onClick={onClick} variants={listItemV}
      ...
      <span style={{ display: "flex", alignItems: "center", gap: "12px" }}>
        {photoSeed && (
          <motion.span style={{ width: "38px", height: "38px", borderRadius: "10px", overflow: "hidden", flexShrink: 0,
            border: `1px solid ${accent || sessionAccent}40` }}
            whileHover={{ scale: 1.08, rotate: -2 }} transition={{ duration: 0.25, ease: [0.22, 1, 0.36, 1] }}>
            <img src={photoFor(photoSeed, 90, 90)} alt="" loading="lazy"
              style={{ width: "100%", height: "100%", objectFit: "cover", filter: "saturate(.7) contrast(1.05)" }} />
          </motion.span>
        )}
        <span style={{ flex: 1 }}>
  ...
  function StageOverlay() {
    ...
    if (stage === "mode") return (
      <Overlay>
        ...
        <motion.div initial="hidden" animate="visible" variants={listContainer}>
          {[{ key: "formal", label: "Formal", accent: ACCENT.formal, sub: "High-stakes — interviews, panels, negotiations", photoSeed: "profess-formal" },
            { key: "social", label: lang === "id" ? "Sosial" : "Social", accent: ACCENT.social, sub: "Interpersonal — friends, family, first impressions", photoSeed: "profess-social" }].map(opt => (
            <ChoiceRow key={opt.key} accent={opt.accent} sub={opt.sub} photoSeed={opt.photoSeed} onClick={() => { setPendingMode(opt.key); setStage("disclaimer"); }}>
              {opt.label}
            </ChoiceRow>
          ))}
        </motion.div>
          ...
          <motion.div initial="hidden" animate="visible" variants={listContainer} style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
            {groups.map((g, gi) => (
              <div key={gi}>
                ...
                {g.items.map((item, ii) => (
                  <ChoiceRow key={ii} photoSeed={`${pendingMode}-${g.group}-${item}`}
  ...
  return (
    <div style={{ minHeight: "100vh", height: "100vh", overflow: "hidden", background: BG, color: "#F0EDE6", fontFamily: "'Inter',sans-serif", fontWeight: 300, position: "relative" }}>
      ...
      {/* Scene photography — a soft, masked photographic "set" bleeding in from
          the edge, tinted to the current scene and crossfading + slowly
          zooming (Ken Burns) on scene change. Pure atmosphere, never sharp
          or literal enough to compete with text. */}
      <div style={{ position: "absolute", inset: 0, zIndex: 0, overflow: "hidden", pointerEvents: "none" }}>
        <AnimatePresence mode="sync">
          <motion.div key={scene.id}
            initial={{ opacity: 0 }} animate={{ opacity: isMobile ? 0.22 : 0.4 }} exit={{ opacity: 0 }}
            transition={{ duration: 1.6, ease: [0.22, 1, 0.36, 1] }}
            style={{ position: "absolute", inset: 0,
              WebkitMaskImage: isMobile
                ? "linear-gradient(to bottom, transparent 0%, black 55%, black 100%)"
                : "linear-gradient(to left, black 0%, black 38%, transparent 78%)",
              maskImage: isMobile
                ? "linear-gradient(to bottom, transparent 0%, black 55%, black 100%)"
                : "linear-gradient(to left, black 0%, black 38%, transparent 78%)" }}>
            <motion.img src={photoFor(scene.id, 1000, 1400)} alt=""
              initial={{ scale: 1.0 }} animate={{ scale: 1.09 }}
              transition={{ duration: 16, ease: "easeOut" }}
              style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center 30%",
                filter: "saturate(.55) brightness(.65) sepia(.18)" }} />
            <div style={{ position: "absolute", inset: 0, background: `linear-gradient(165deg, ${scene.top}55 0%, ${scene.bottom}aa 70%, #0A0807cc 100%)`,

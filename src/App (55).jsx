...
function RiveCoach({ talking, listening }) {
  ...
  return (
    <div className="char-enter" style={{ width: "100%", height: "100%", animation: "breathe 5s ease-in-out infinite" }}>
...
export default function Profess() {
  ...
  const backBtn = (to) => (
    <button onClick={() => setStage(to)} style={{ background: "none", border: "none", color: "#9C8E7C", fontSize: "10px", letterSpacing: ".08em", textTransform: "uppercase", display: "flex", alignItems: "center", gap: "5px", padding: 0, marginBottom: "18px" }}>
      <IconArrowLeft /> Back
    </button>
  );
    ...
    if (stage === "scenario") {
      ...
      return (
        <Overlay width="min(560px, 90vw)">
          ...
          <button onClick={() => { setScenario(null); setSceneGroupIdx(null); startSession(pendingMode, null); }}
      ...
      {inConversation && (
        <>
          <div style={{ position: "absolute", top: 0, left: 0, right: 0, padding: "20px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", zIndex: 10 }}>
            <button onClick={() => setStage("intro")} style={{ background: "none", border: "none", color: "#9C8E7C", fontSize: "13px", fontFamily: "'Inter',sans-serif" }}>Profess</button>
            <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
              <button onClick={() => setTranscriptOpen(o => !o)} style={{ background: "none", border: "1px solid #2A241C", borderRadius: "8px", color: "#AFA290", fontSize: "11px", padding: "6px 10px" }}>
                {transcriptOpen ? (lang === "id" ? "Sembunyikan" : "Hide transcript") : (lang === "id" ? "Transkrip" : "Transcript")}
              </button>
              <button onClick={() => setSpeechEnabled(s => !s)} style={{ background: "none", border: "none", color: "#AFA290" }}>{speechEnabled ? <IconVolume/> : <IconMute/>}</button>
              <button onClick={resetSession} style={{ background: "none", border: "none", color: "#AFA290" }}><IconRefresh/></button>
            </div>
          </div>
          ...
          <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, padding: "16px 28px 22px", zIndex: 10, background: "linear-gradient(to top, rgba(7,8,9,0.92), transparent)" }}>
            <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px", marginBottom: "8px" }}>
              {lastExchange && <button onClick={tryAgain} style={{ background: "none", border: "none", color: "#9C8E7C", fontSize: "11px" }}>{lang === "id" ? "Coba lagi" : "Try again"}</button>}
              <button onClick={endSession} style={{ background: "none", border: "none", color: "#9C8E7C", fontSize: "11px" }}>{lang === "id" ? "Akhiri" : "End session"}</button>
            </div>
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "flex-end" }}>
              <textarea ref={textareaRef} value={input} onChange={handleTA} onKeyDown={handleKeyDown}
                ...
                style={{ flex: 1, minHeight: "48px", maxHeight: "160px", resize: "none", background: "#1A1510",
              ...
              {isSpeaking && <button onClick={stopSpeech} style={{ background: "#1A1510", border: "1px solid #2A241C", borderRadius: "10px", padding: "12px", color: "#AFA290" }}><IconStop/></button>}
              <button onClick={toggleMic} className={isListening ? "mic-active" : ""} style={{ background: "#1A1510", border: "1px solid #2A241C", borderRadius: "10px", padding: "12px", color: isListening ? "#BC5A5A" : "#AFA290" }}><IconMic/></button>
              <button id="psend" onClick={sendMessage} disabled={loading || !input.trim()} style={{ background: sessionAccent, border: "none", borderRadius: "10px", padding: "12px 16px", color: "#15110C", opacity: loading || !input.trim() ? .4 : 1 }}><IconSend/></button>
            </div>
          </div>
        </>
      )}

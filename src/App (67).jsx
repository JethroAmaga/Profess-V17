...
export default function Profess() {
  ...
  const galleryChars = useMemo(() => {
    ...
    const roleEntries = Object.keys(ROLE_TITLES).map((key) => {
      ...
      return { key, title: ROLE_TITLES[key], svg: buildSVG(ch, "neutral", false) };
    });
    return [{ key: "coach", title: CHARS.default.title, svg: buildSVG(CHARS.default, "neutral", false) }, ...roleEntries];
  }, [svgGallery]);
      ...
      <AnimatePresence>
        {inConversation && (
          <motion.div
            ...
            animate={{ left: isMobile ? "50%" : "94%", top: isMobile ? "8%" : "50%", x: isMobile ? "-50%" : "-100%", y: isMobile ? "0%" : "-50%", opacity: 1, scale: 1 }}
            ...
            <motion.div style={{ scale: lean }}>
              <CharacterFigure size={isMobile ? "min(46vh, 320px)" : "min(58vh, 520px)"} />
            </motion.div>

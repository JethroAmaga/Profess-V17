...
// Trial: a single free, no-attribution motion-loop clip (Coverr) used as the
// scene backdrop instead of a static photo. Served from /public, so no
// external network host is involved at all.
...
const photoFor = (seed, w = 1000, h = 1400) => {
  ...
  return `https://loremflickr.com/${w}/${h}?lock=${lock % 100000}`;
};
...
export default function Profess() {
  ...
  useEffect(() => { setPhotoFailed(false); }, [scene.id]);
  // Trial: real video-loop backdrop (see VIDEO_LOOP_SRC) in place of the photo.
  // Independent of scene.id since there's currently only one clip — it doesn't
  // reset per scene like the photo fallback does.
        ...
        <AnimatePresence mode="sync">
          <motion.div key={scene.id}
            ...
            <motion.div style={{ position: "absolute", inset: "-4%", x: photoX, y: photoY }}>
              ...
              {videoFailed ? (
                <motion.img
                  src={photoFailed ? sceneArtFor(scene.id, 1200, 1600, scene) : photoFor(scene.id, 1200, 1600)}
                ...
                // Trial: real motion-loop footage instead of a static photo + simulated
                // Ken-Burns zoom. The clip already carries its own camera move, so we
                // don't re-animate scale here — just crossfade it in and let it loop.
                // Falls back to the photo backdrop if the video can't load/decode.
                <motion.video
                  src={VIDEO_LOOP_SRC}

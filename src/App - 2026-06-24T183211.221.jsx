        ...
        {inConversation && (
          <motion.div
            ...
            <div style={{ position: "absolute", top: "100%", marginTop: isMobile ? "6px" : "14px", left: "50%", transform: "translateX(-50%)", textAlign: "center", whiteSpace: "nowrap" }}>
              <TiltBox style={{ boxSizing: "border-box", padding: "8px 18px", borderRadius: "12px",
                background: BG, border: "1px solid #3A3024", boxShadow: "0 10px 24px -14px rgba(0,0,0,.55)" }}>
                <p style={{ fontFamily: "'Playfair Display',serif", fontStyle: "italic", fontSize: "18px", color: "#F0EDE6" }}>{charMeta.name || charMeta.title}</p>
                <p style={{ fontSize: "11px", color: "#F5F2EC", marginTop: isMobile ? "0px" : "-4px" }}>{charMeta.title}</p>
        ...
        <>
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}
            ...
            <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
              <motion.button onClick={() => setTranscriptOpen(o => !o)}
                whileHover={{ borderColor: `${sessionAccent}88`, color: "#000" }} whileTap={{ scale: 0.96 }}
                transition={{ duration: 0.18 }}
                style={{ background: "none", border: "1px solid #2A241C", borderRadius: "8px", color: "#000", fontWeight: 700, fontSize: "11px", padding: "6px 10px" }}>
          ...
          <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, padding: "16px 28px 22px", zIndex: 10, background: "linear-gradient(to top, rgba(7,8,9,0.92), transparent)" }}>
            <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px", marginBottom: "8px" }}>
              {lastExchange && <motion.button onClick={tryAgain} whileHover={{ opacity: 0.7 }} whileTap={{ scale: 0.95 }} style={{ background: "none", border: "none", color: "#000", fontWeight: 700, fontSize: "11px" }}>{lang === "id" ? "Coba lagi" : "Try again"}</motion.button>}
              <motion.button onClick={endSession} whileHover={{ opacity: 0.7 }} whileTap={{ scale: 0.95 }} style={{ background: "none", border: "none", color: "#000", fontWeight: 700, fontSize: "11px" }}>{lang === "id" ? "Akhiri" : "End session"}</motion.button>
            </div>
